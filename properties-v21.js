 ...
 * Properties Registry Controller (v21 - Toast Notifications for Uploads)
 */
console.log("JB Properties Controller v21 initializing...");
...
const JoeBuildsProperties = (() => {
  ...
  const handleNewPropertySubmit = async () => {
    ...
    } else { showToast('Error creating property', 'error'); }
  };
  ...
  const handleSaveProperty = async () => {
    ...
    if(!error) { 
      ...
      supabase.from('buildings').select('*').then(({data}) => {
        if(data) {
          updateDatalist('state-options', data.map(b => b.state), AU_STATES);
  ...
  const fetchBuildingDetails = async (buildingId) => {
    ...
    if (DOM.detailRoomsList) {
      ...
      if (currentBuildingRooms.length === 0) {
        DOM.detailRoomsList.innerHTML = `<div style="border:1px dashed var(--border);background-color:var(--surface);padding:1.5rem;text-align:center;font-family:var(--font-mono);font-size:10px;text-transform:uppercase;color:var(--muted-foreground)">No zones initialized yet</div>`;
      } else {
  ...
  const init = async () => {
    try {
      ...
      DOM.detailFilesList = document.getElementById('detail-files-list'); DOM.detailFilesCount = document.getElementById('detail-files-count'); DOM.fileInput = document.getElementById('file-upload-input'); DOM.uploadStatusText = document.getElementById('upload-status-text');
