 ...
 * Administrative Data Controller (RBAC Enforced)
 */
const JoeBuildsAdmin = (() => {
  ...
  const initUIEvents = () => {
    ...
    DOM.assetSelect?.addEventListener('change', (e) => { currentBuildingId = e.target.value; loadAssetContext(currentBuildingId); });
  ...
  const generateStatusOptions = (currentStatus) => {
    ...
    return options.map(opt => {
      const label = opt === 'risk' ? 'At Risk' : opt === 'review' ? 'Review Required' : opt.charAt(0).toUpperCase() + opt.slice(1);
      return `<option value="${opt}" ${opt === currentStatus ? 'selected' : ''}>${label}</option>`;
    }).join('');
  };
  ...
  const renderTwinEditor = (rooms, measurements) => {
    ...
    rooms.forEach(room => {
      const roomMs = measurements.filter(x => x.room_id === room.id);
      const overallM = roomMs.find(x => x.status_flag === 'risk' || x.status_flag === 'review') || roomMs[0];
      if (!overallM) return;
      const getValInput = (code) => { const sm = roomMs.find(x => x.measurement_points?.element_code === code); if(!sm) return `<input class="control-input tabular" disabled value="-">`; return `<input step="any" class="control-input tabular" type="number" value="${sm.value}" data-table="measurements" data-id="${sm.id}" data-col="value">`; };
