 ...
 * Diagnostics Controller (v5 - Sidebar Meta Fix & Skeleton Loader)
 */
const JoeBuildsDiagnostics = (() => {
  ...
  const DOM = {
    ...
    // ROCK SOLID SELECTOR FOR SIDEBAR META ("Authenticating" text)
    sidebarMeta: document.getElementById('desktopSidebarMeta') || document.querySelector('.sidebar-footer div:last-child'),
  ...
  const generateThermalSVG = (metric, value) => { return `<svg viewBox="0 0 400 250" style="height:100%;width:100%" preserveAspectRatio="none"><defs><radialGradient id="th" cx="35%" cy="55%" r="55%"><stop offset="0%" stop-color="oklch(0.72 0.18 60)"/><stop offset="35%" stop-color="oklch(0.55 0.16 30)"/><stop offset="65%" stop-color="oklch(0.35 0.14 280)"/><stop offset="100%" stop-color="oklch(0.18 0.06 260)"/></radialGradient></defs><rect width="400" height="250" fill="url(#th)"/><text x="12" y="18" font-family="monospace" font-size="9" fill="rgba(255,255,255,0.7)">FLIR · 320x240 · ε 0.95</text><text x="12" y="240" font-family="monospace" font-size="9" fill="rgba(255,255,255,0.7)">${metric}</text><text x="328" y="240" font-family="monospace" font-size="9" fill="rgba(255,255,255,0.7)">${value}</text></svg><div style="position:absolute;right:.75rem;top:.75rem;border:1px solid rgba(255,255,255,.2);background:rgba(0,0,0,.4);padding:.25rem .5rem;color:rgba(255,255,255,.8);font-size:9px;text-transform:uppercase" class="jb-font-mono"><svg style="width:12px;height:12px;display:inline-block;vertical-align:middle;margin-right:.25rem"><use href="#i-zoom"/></svg>1× zoom</div>`; };
  const generateIAQSVG = (metric, value) => { return `<svg viewBox="0 0 400 250" style="height:100%;width:100%" preserveAspectRatio="none"><defs><linearGradient id="iaq-grad" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="rgba(132, 197, 148, 0.4)"/><stop offset="100%" stop-color="rgba(132, 197, 148, 0)"/></linearGradient></defs><rect width="400" height="250" fill="#111512"/><path d="M 0 50 L 400 50 M 0 100 L 400 100 M 0 150 L 400 150 M 0 200 L 400 200" stroke="rgba(255,255,255,0.05)" stroke-width="1"/><path d="M 0 160 L 50 130 L 100 145 L 150 110 L 200 125 L 250 90 L 300 105 L 350 80 L 400 95 L 400 250 L 0 250 Z" fill="url(#iaq-grad)"/><path d="M 0 160 L 50 130 L 100 145 L 150 110 L 200 125 L 250 90 L 300 105 L 350 80 L 400 95" fill="none" stroke="#84C594" stroke-width="2"/><text x="12" y="18" font-family="monospace" font-size="9" fill="rgba(255,255,255,0.7)">AWAIR OMNI · CONTINUOUS TELEMETRY</text><text x="12" y="240" font-family="monospace" font-size="9" fill="rgba(255,255,255,0.7)">${metric}</text><text x="328" y="240" font-family="monospace" font-size="9" fill="rgba(255,255,255,0.7)">${value}</text></svg><div style="position:absolute;right:.75rem;top:.75rem;border:1px solid rgba(255,255,255,.2);background:rgba(0,0,0,.4);padding:.25rem .5rem;color:rgba(255,255,255,.8);font-size:9px;text-transform:uppercase" class="jb-font-mono"><svg style="width:12px;height:12px;display:inline-block;vertical-align:middle;margin-right:.25rem"><use href="#i-zoom"/></svg>Live Node</div>`; };
  const generateAirflowSVG = (metric, value) => { return `<svg viewBox="0 0 400 250" style="height:100%;width:100%" preserveAspectRatio="none"><rect width="400" height="250" fill="#161c18"/><path d="M -50 125 Q 100 50 200 125 T 450 125" fill="none" stroke="rgba(66, 117, 138, 0.15)" stroke-width="30"/><path d="M -50 125 Q 100 80 200 125 T 450 125" fill="none" stroke="rgba(66, 117, 138, 0.4)" stroke-width="10"/><path d="M -50 125 Q 100 100 200 125 T 450 125" fill="none" stroke="#42758A" stroke-width="3"/><text x="12" y="18" font-family="monospace" font-size="9" fill="rgba(255,255,255,0.7)">RETROTEC · 50 PA DEPRESSURIZATION</text><text x="12" y="240" font-family="monospace" font-size="9" fill="rgba(255,255,255,0.7)">${metric}</text><text x="328" y="240" font-family="monospace" font-size="9" fill="rgba(255,255,255,0.7)">${value}</text></svg><div style="position:absolute;right:.75rem;top:.75rem;border:1px solid rgba(255,255,255,.2);background:rgba(0,0,0,.4);padding:.25rem .5rem;color:rgba(255,255,255,.8);font-size:9px;text-transform:uppercase" class="jb-font-mono"><svg style="width:12px;height:12px;display:inline-block;vertical-align:middle;margin-right:.25rem"><use href="#i-zoom"/></svg>Flow Vector</div>`; };
  ...
  const toggleSkeletonState = (isLoading) => {
    // ADDED DOM.sidebarMeta TO SKELETON ENGINE
    const elementsToToggle = [
      DOM.desktopSidebarAsset, DOM.desktopHeaderAsset, DOM.headerClimateText, DOM.headerSysText, DOM.countThermal, DOM.countAir, DOM.sidebarMeta
    ];
    ...
    if (isLoading && DOM.tbodyThermal) { DOM.tbodyThermal.innerHTML = `<tr><td colspan="5"><div class="jb-skeleton-block" style="height: 40px; width: 100%;"></div></td></tr>`; DOM.mgridThermal.innerHTML = `<div class="jb-skeleton-block" style="height: 100px; width: 100%; margin-bottom: 4px;"></div>`; if(DOM.tbodyAir) DOM.tbodyAir.innerHTML = `<tr><td colspan="5"><div class="jb-skeleton-block" style="height: 40px; width: 100%;"></div></td></tr>`; if(DOM.mgridAir) DOM.mgridAir.innerHTML = `<div class="jb-skeleton-block" style="height: 100px; width: 100%; margin-bottom: 4px;"></div>`; }
  };
  ...
  const mapToDOM = (data) => {
    ...
    // UPDATE CLIMATE & SIDEBAR META DYNAMICALLY
    ...
    if (DOM.tbodyThermal) DOM.tbodyThermal.innerHTML = ''; if (DOM.mgridThermal) DOM.mgridThermal.innerHTML = '';
    if (DOM.tbodyAir) DOM.tbodyAir.innerHTML = ''; if (DOM.mgridAir) DOM.mgridAir.innerHTML = '';

    const thermalCodes = ['U-VALUE', 'THERMAL_BRIDGE', 'ENVELOPE']; const airCodes = ['ACH50', 'ELA', 'CO2'];
    const thermals = [], airTights = [];
    
    data.measurements.forEach(m => {
      measurementsStore[m.id] = m; const code = m.measurement_points?.element_code;
      if (thermalCodes.includes(code)) thermals.push(m); if (airCodes.includes(code)) airTights.push(m);
    });

    if (DOM.tbodyThermal && DOM.mgridThermal && DOM.countThermal) {
      DOM.countThermal.textContent = `${thermals.length} entries`;
      thermals.forEach(m => {
        let title = 'Metric'; if(m.measurement_points.element_code === 'U-VALUE') title = 'Assembly U-Value'; if(m.measurement_points.element_code === 'THERMAL_BRIDGE') title = 'Linear Thermal Bridging (ψ)'; if(m.measurement_points.element_code === 'ENVELOPE') title = 'Envelope Integrity Index';
        DOM.tbodyThermal.insertAdjacentHTML('beforeend', renderRowHTML(m, title)); DOM.mgridThermal.insertAdjacentHTML('beforeend', renderMobileCardHTML(m, title));
      });
    }
    ...
    if (DOM.tbodyAir && DOM.mgridAir && DOM.countAir) {
      ...
      airTights.forEach(m => {
        let title = 'Metric'; if(m.measurement_points.element_code === 'ACH50') title = 'ACH50 Air Infiltration Rate'; if(m.measurement_points.element_code === 'ELA') title = 'Envelope Leakage Area (ELA)'; if(m.measurement_points.element_code === 'CO2') title = 'Indoor Air Quality (IAQ)';
        DOM.tbodyAir.insertAdjacentHTML('beforeend', renderRowHTML(m, title)); DOM.mgridAir.insertAdjacentHTML('beforeend', renderMobileCardHTML(m, title));
      });
    }

    document.querySelectorAll('tr[data-id], .jb-mobile-data-card[data-id]').forEach(el => {
      el.addEventListener('click', (e) => { e.stopPropagation(); const metricName = el.querySelector('td:nth-child(2)')?.textContent || el.querySelector('.jb-mobile-card-header div').textContent; openModal(el.getAttribute('data-id'), metricName); });
    });
  ...
  const openModal = (id, metricName) => {
    const m = measurementsStore[id]; if (!m) return;
    const code = m.measurement_points?.element_code; const valString = `${m.value || ''} ${m.unit || ''}`.trim();
    DOM.mTitle.textContent = metricName; DOM.mLabel.textContent = metricName.toUpperCase(); DOM.mValue.textContent = valString;
    const d = new Date(m.created_at); DOM.mCapture.textContent = `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')} ${String(d.getHours()).padStart(2,'0')}:${String(d.getMinutes()).padStart(2,'0')}`;
    
    if (DOM.modalGraphicWrapper) {
      if (code === 'CO2') { DOM.modalGraphicWrapper.innerHTML = generateIAQSVG(metricName.toUpperCase(), valString); DOM.mInst.textContent = 'Awair Element'; DOM.mOp.textContent = 'Automated Telemetry Node'; } 
      else if (code === 'ACH50' || code === 'ELA') { DOM.modalGraphicWrapper.innerHTML = generateAirflowSVG(metricName.toUpperCase(), valString); DOM.mInst.textContent = 'Retrotec 3000'; DOM.mOp.textContent = 'Airtightness Technician'; } 
      else { DOM.modalGraphicWrapper.innerHTML = generateThermalSVG(metricName.toUpperCase(), valString); DOM.mInst.textContent = 'FLIR E76'; DOM.mOp.textContent = 'Thermography Specialist'; }
    }
  ...
  const init = async () => {
    ...
    try { const profile = await authAndGetProfile(); if (profile?.building_id) { const diagnosticsData = await fetchDiagnosticsData(profile.building_id); mapToDOM(diagnosticsData); } else { toggleSkeletonState(false); } } catch (error) { window.location.href = '/login'; }
  };
  return { init };
})();
