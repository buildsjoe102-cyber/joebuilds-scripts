 ...
 * Digital Twin Controller (v11 - Dynamic Database Coordinates)
 */
const JoeBuildsDigitalTwin = (() => {
  ...
  const roomCoordinates = {
    'primary': { left: '22%', top: '28%' }, 'kitchen': { left: '60%', top: '32%' },
    'plant': { left: '78%', top: '62%' }, 'subfloor': { left: '40%', top: '78%' },
  ...
  const processZoneData = (rooms, measurements) => {
    ...
    rooms.forEach(room => {
      ...
      zones[room.id] = { 
        ...
        map_x: room.map_x, map_y: room.map_y, // New Coordinate Data
        ...
        notes: room.notes || 'No active analyst notes for this zone.', monitoring: monitoringText
      };
    });
    return zones;
  };
  ...
  const clearActiveStates = () => { document.querySelectorAll('.jb-hotspot').forEach(el => el.classList.remove('jb-active-node')); document.querySelectorAll('.jb-zone-row-item').forEach(el => el.classList.remove('jb-active-node')); };

  const openDiagnosticPanel = (zoneId) => {
    clearActiveStates(); const data = activeZonesData[zoneId]; if (!data) return;
  ...
  const init = async () => {
    ...
    try { const profile = await authAndGetProfile(); if (profile?.building_id) { const twinData = await fetchTwinData(profile.building_id); renderDashboard(twinData); } else { toggleSkeletonState(false); } } catch (error) { window.location.href = '/login'; }
  };
  return { init };
})();
