 ...
 * Global Profile Manager (v8 - Total Mobile Nav Destruction for Clients)
 */
const JoeBuildsProfileManager = (() => {
  ...
  const init = async () => {
    ...
    if (window.supabase) {
      ...
      injectGlobalCSS(); injectProfileHTML(); bindProfileEvents();
      ...
      try {
        ...
        if (member && member.data) {
          ...
          if (currentUserRole === 'client' || currentUserRole === 'demo') {
            // PHYSICALLY DESTROY THE BOTTOM NAVBAR FOR CLIENTS
