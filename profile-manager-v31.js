 ...
 * Global Profile Manager (v31 - Multi-Property Client Switcher)
 */
const JoeBuildsProfileManager = (() => {
  ...
  const bindClientPropertySwitcher = async (profile) => {
    if (profile.role === 'admin' || profile.role === 'operator') return; // Admins use the Admin panel selection
    ...
    if (validAccesses.length > 1) {
       // Find the header element that normally says "PRJ-000"
       ...
       if (headerAssetEl) {
           ...
           // Replace it with a sleek dropdown that blends into the header
           headerAssetEl.innerHTML = `<select id="clientPropertySwitcher" style="background:transparent; border:1px solid rgba(255,255,255,0.2); border-radius:4px; color:inherit; font-family:inherit; font-size:10px; cursor:pointer; outline:none; font-weight:bold; padding:2px 4px; margin-left:4px;">${optionsHtml}</select>`;
           ...
           document.getElementById('clientPropertySwitcher').addEventListener('change', async (e) => {
               ...
               window.location.reload(); // Instantly reload to fetch the new property data across the page
           });
       }
    }
  };
    ...
    if (window.supabase) {
      ...
      try {
        ...
        if (currentMember && currentMember.data) {
          ...
          await bindClientPropertySwitcher(profile); // Check if they have multiple properties!
