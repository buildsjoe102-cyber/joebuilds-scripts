 ...
 * Unified Digital Twin Controller (v4 - Dynamic Sidebar Meta)
 */
const JoeBuildsDigitalTwin = (() => {
  ...
  const toggleSkeletonState = (isLoading) => {
    ...
    if (isLoading && DOM.zonesListContainer) {
      DOM.zonesListContainer.innerHTML = `<div class="jb-zone-row-item jb-skeleton-block" style="height: 48px; margin-bottom: 1px;"></div><div class="jb-zone-row-item jb-skeleton-block" style="height: 48px; margin-bottom: 1px;"></div><div class="jb-zone-row-item jb-skeleton-block" style="height: 48px; margin-bottom: 1px;"></div>`;
    }
  };
  ...
  const renderDashboard = (data) => {
    ...
    DOM.hotspots.forEach(h => { 
      ...
      if (!activeZonesData[code]) h.style.display = 'none'; 
      else { h.style.display = 'flex'; const newH = h.cloneNode(true); h.parentNode.replaceChild(newH, h); newH.addEventListener('click', (e) => { e.stopPropagation(); openDiagnosticPanel(code); }); }
    });
