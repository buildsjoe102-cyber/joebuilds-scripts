 ...
 * Diagnostics Controller (v25 - Room Context & Deduplication)
 */
console.log("JB Diagnostics Controller v25 initializing...");
...
const JoeBuildsDiagnostics = (() => {
  ...
  const mapToDOM = (bData, mData, rData) => {
    ...
    // Create a dictionary of rooms to look up room names
    const roomsMap = {};
    if (rData) {
        rData.forEach(r => { roomsMap[r.id] = r.room_name_current; });
    }
    ...
    // Deduplicate: Only keep the LATEST measurement for each Element Code + Room combo
    const seen = new Set();
    ...
    if (thermals.length === 0) {
      ...
      thermals.forEach(m => { 
        ...
        // Append Room Name if it belongs to a room
        if (m.room_id && roomsMap[m.room_id]) {
            title += ` <span style="color:var(--muted-foreground); font-family:var(--font-mono); font-size:9px; letter-spacing:0.05em; margin-left:6px; padding-left:6px; border-left:1px solid var(--border);">ZONE: ${roomsMap[m.room_id]}</span>`;
        }
    ...
    document.querySelectorAll('tr[data-id], .jb-mobile-data-card[data-id]').forEach(el => { 
      el.addEventListener('click', (e) => { 
        ...
        // Strip out the "ZONE: " tag for the modal title so it looks cleaner
        if (metricName.includes('ZONE:')) {
            metricName = metricName.split('ZONE:')[0].trim();
        }
  ...
  const init = async () => {
    try {
      ...
      if (targetBuildingId) { 
        // Fetch building, measurements, AND rooms concurrently
