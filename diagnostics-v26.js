 ...
 * Diagnostics Controller (v26 - Header Shimmer Integration)
 */
console.log("JB Diagnostics Controller v26 initializing...");
...
const JoeBuildsDiagnostics = (() => {
  ...
  const toggleSkeletonState = (isLoading) => {
    // Shimmer all tabular data spans in the header (including CLIMATE & STATUS)
    document.querySelectorAll('.header-telemetry-cluster .tabular, .jb-desktop-telemetry .tabular').forEach(el => {
  ...
  const mapToDOM = (bData, mData, rData) => {
    ...
    // Explicitly target the 3 header nodes by index
    const headerTabs = document.querySelectorAll('.header-telemetry-cluster .tabular, .jb-desktop-telemetry .tabular');
