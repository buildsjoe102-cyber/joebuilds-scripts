 ...
 * Digital Twin Controller (v10 - Fixed Modal Visibility & Slide Animation)
 */
const JoeBuildsDigitalTwin = (() => {
  ...
  const initUIEvents = () => {
    ...
    const closeAside = () => {
      ...
      // Wait for the slide-out animation to finish, then fully hide it from the DOM
      setTimeout(() => {
  ...
  const openDiagnosticPanel = (zoneId) => {
    ...
    // CRITICAL FIX: Make sure the element is physically visible before sliding it in!
    DOM.aside.classList.remove('jb-hidden');
    
    // Tiny delay to force the browser to register the visibility change before starting the transition
    setTimeout(() => {
