 ...
 * Global Profile Manager (v37 - RLS Sync & Dropdown CSS Fixes)
 */
const JoeBuildsProfileManager = (() => {
  ...
  const bindClientPropertySwitcher = async (profile) => {
    // Only clients use this feature. Admins use the actual Admin page.
      ...
      try {
        ...
        if (currentMember && currentMember.data) {
          ...
          if (opEmail) {
              ...
              // Force CSS styling directly to prevent awkward wrapping
              opEmail.style.cssText = "font-size: 10px !important; text-transform: none !important; letter-spacing: normal !important; color: var(--muted-foreground, #637066) !important; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; padding-bottom: 0.5rem;";
          }
