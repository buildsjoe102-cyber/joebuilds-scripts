/**
 * Joe Builds - Reports Controller (v9 - JWT Security & Calendar)
 */
const JoeBuildsReports = (() => {
  ...
  const DOM = { 
    ...
    reportsGrid: document.getElementById('reportsGrid'), reportsCountLabel: document.getElementById('reportsCountLabel'), calendarMonthLabel: document.getElementById('calendarMonthLabel'), calendarMatrix: document.getElementById('calendarMatrix'), agendaContainer: document.getElementById('agendaContainer') 
  };
  ...
  const toggleSkeletonState = (isLoading) => { 
    ...
    if (isLoading) { 
      if (DOM.reportsGrid) DOM.reportsGrid.innerHTML = `<div class="archive-row jb-skeleton-block" style="height: 70px;"></div>`; 
      ...
      if (DOM.agendaContainer) DOM.agendaContainer.innerHTML = `<div class="agenda-item jb-skeleton-block" style="height: 48px;"></div>`; 
    } 
  };
  ...
  const init = async () => {
    ...
    if (pageTitle) pageTitle.textContent = "Reports & Stability Checks";
    const pageDesc = document.querySelector('.hero-desc');
    if (pageDesc) pageDesc.textContent = "Your home reports, measurement records, and upcoming stability checks are stored here.";
    const auditLabel = document.querySelector('.calendar-header-row .text-muted-foreground');
    if (auditLabel) auditLabel.textContent = "Stability Check Cycle";

    if (!window.supabase) return; 

    // SECURE TOKEN HANDSHAKE
    let supabaseToken = '';
    try { supabaseToken = await window.$memberstackDom.getProviderToken('supabase'); } 
    ...
    try {
      ...
      if (targetBuildingId) { 
        const pageData = await fetchPageData(targetBuildingId); 
