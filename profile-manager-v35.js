 ...
 * Global Profile Manager (v35 - Anti-Overwrite Property Switcher)
 */
const JoeBuildsProfileManager = (() => {
  ...
  const bindClientPropertySwitcher = async (profile) => {
    // Admins don't need this, they have the big dropdown in the Admin panel
    ...
    if (validAccesses.length > 1) {
       ...
       if (headerTabs && headerTabs[0]) {
           // 1. THIS IS THE FIX: Hide the original span so it can't be overwritten!
           headerTabs[0].style.display = 'none';
