 ...
 * Administrative Data Controller (v5 - Strict User Modal RBAC)
 */
const JoeBuildsAdmin = (() => {
  ...
  const initUIEvents = () => {
    ...
    if (DOM.closeUsersModal) DOM.closeUsersModal.addEventListener('click', () => DOM.usersModal.classList.add('jb-hidden'));
  ...
  const authenticateAdmin = async () => {
    ...
    currentUserRole = profile.role; // Store global role state
    ...
    // Both Admins and Operators can see the Users button
  ...
  const loadAssetContext = async (buildingId) => {
    ...
    DOM.desktopSidebarAsset.textContent = bTitle; DOM.desktopHeaderAsset.textContent = bTitle;
    ...
    if (measurementsRes.data.length === 0 && scenariosRes.data.length === 0) {
      DOM.editorsWrap.classList.add('jb-hidden');
      DOM.emptyStateContainer.classList.remove('jb-hidden');
    } else {
      DOM.emptyStateContainer.classList.add('jb-hidden');
      DOM.editorsWrap.classList.remove('jb-hidden');
  ...
  /**
   * USER MANAGEMENT MODAL LOGIC (RBAC Enforced)
   */
  const openUsersModal = async () => {
    ...
    profiles.forEach(p => {
      ...
      if (p.building_id) tr.querySelector(`select[data-type="building"]`).value = p.building_id;
