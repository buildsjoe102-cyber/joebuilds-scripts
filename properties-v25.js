/**
 * Joe Builds - Properties Registry Controller (v25 - Dropdown Fix & Delete Property)
 */
console.log("JB Properties Controller v25 initializing...");
const JoeBuildsProperties = (() => {
  ...
  // AUTO-FIX: Converts old Webflow <select> elements into typing-enabled <input list> elements
  const fixEditModalDropdowns = () => {
    const editFields = [
      { id: 'edit-state', list: 'state-options' },
      { id: 'edit-status', list: 'status-options' },
      { id: 'edit-property-type', list: 'type-options' }
    ];
  ...
  const openEditPropertyModal = () => {
    ...
    if (form && !document.getElementById('btnDeleteProperty')) {
      ...
      delBtn.onmouseover = () => delBtn.style.background = 'rgba(166, 68, 68, 0.05)';
  ...
  const handleSaveProperty = async () => {
    ...
    const payload = { 
      ...
      property_type: propTypeInput ? propTypeInput.value : '' 
    };
  ...
  const init = async () => {
    try {
      ...
      // AUTOMATICALLY FIX OLD SELECT DROPDOWNS TO SUPPORT CUSTOM TYPING
