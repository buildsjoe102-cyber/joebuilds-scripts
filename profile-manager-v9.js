 ...
 * Global Profile Manager (v9 - Desktop Mobile-Nav Vaporizer & Help System)
 */
const JoeBuildsProfileManager = (() => {
  ...
  const injectGlobalCSS = () => {
    ...
    style.innerHTML = `
      ...
      .jb-accordion-content strong { color: var(--foreground, #1A241D); font-weight: 600; }
      .jb-accordion-content ul { padding-left: 1.5rem; margin-top: 0.5rem; margin-bottom: 0.5rem; }
      .jb-accordion-content li { margin-bottom: 0.25rem; }
      ...
      /* ========================================================= */
      /* THE DESKTOP VAPORIZER: FORCES MOBILE NAV OFF DESKTOP      */
      /* ========================================================= */
    ...
    if (role === 'admin' || role === 'operator') {
      return `
        <div class="jb-accordion-item is-open">
          <button class="jb-accordion-header">Continuous Auto-Save Mechanics <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m6 9 6 6 6-6"></path></svg></button>
          <div class="jb-accordion-content">
            This platform uses continuous sync architecture. <strong>There are no "Save" buttons</strong> for telemetry inputs, statuses, or pathway milestones.<br><br>
            <strong>How to save:</strong> Type your value or select a status, and simply <strong>click anywhere outside the field</strong> (or tap outside on mobile), or press <strong>Enter</strong>. <br><br>
            The background of the card will briefly pulse green to confirm the data has been securely written to the database and instantly published to the client.
          </div>
        </div>

        <div class="jb-accordion-item">
          <button class="jb-accordion-header">Managing Properties & Rooms <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m6 9 6 6 6-6"></path></svg></button>
          <div class="jb-accordion-content">
            Navigate to the <strong>Properties</strong> page to register new assets. When viewing a property's details, you can add <strong>Spatial Nodes (Rooms)</strong>.<br><br>
            <strong>Automation:</strong> Creating a new room automatically generates blank RH%, CO₂, and VOC sensors in the database for that specific room, instantly making them editable on the Admin page and viewable on the Digital Twin.
          </div>
        </div>

        <div class="jb-accordion-item">
          <button class="jb-accordion-header">Diagnostic Report Uploads <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m6 9 6 6 6-6"></path></svg></button>
          <div class="jb-accordion-content">
            Files dragged into the <strong>Admin Page Uploader</strong> are immediately published to the client's Reports center. Files uploaded on the <strong>Properties Page</strong> are considered internal evidence and are hidden from the client view.
          </div>
        </div>

        ${role === 'admin' ? `
        <div class="jb-accordion-item">
          <button class="jb-accordion-header">User & Access Management <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m6 9 6 6 6-6"></path></svg></button>
          <div class="jb-accordion-content">
            Click the <strong>Users</strong> button in the top header. Here, you can assign clients to their properties so their dashboard populates.<br><br>
            As an Admin, you are the only role permitted to elevate a user's permissions from "Client" to "Operator" or "Admin". Changes save automatically when a new dropdown value is selected.
          </div>
        </div>` : ''}
      `;
    } else {
      ...
      return `
        <div class="jb-accordion-item is-open">
          <button class="jb-accordion-header">Understanding Your Dashboard <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m6 9 6 6 6-6"></path></svg></button>
          <div class="jb-accordion-content">
            Your dashboard provides a high-level health summary of your building. <br><br>
            <strong>Integrity Index:</strong> The badge in the top right is a composite score out of 1.00 indicating the overall resilience of your building envelope.<br><br>
            <strong>Next Action:</strong> The dark green panel at the bottom highlights the exact next step required in your engineering sequence. Clicking "Review Plan" will show you the full roadmap.
          </div>
        </div>
        ...
        <div class="jb-accordion-item">
          ...
          <div class="jb-accordion-content">
            <ul>
              <li><strong>ACH50:</strong> Air Changes per Hour at 50 Pascals. This measures how drafty or airtight your home is. Lower is better.</li>
              <li><strong>U-Value:</strong> Measures thermal transmittance. It tells us how effectively your walls and roof prevent heat from escaping. Lower is better.</li>
              <li><strong>VOC (Volatile Organic Compounds):</strong> Harmful gasses emitted by paints, glues, and materials that impact indoor air quality.</li>
              <li><strong>RH (Relative Humidity):</strong> The amount of moisture in the air. High RH leads to condensation and structural mold risk.</li>
            </ul>
          </div>
        </div>

        <div class="jb-accordion-item">
          <button class="jb-accordion-header">Digital Twin & Diagnostics <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m6 9 6 6 6-6"></path></svg></button>
          <div class="jb-accordion-content">
            <strong>Digital Twin:</strong> An interactive map of your property. Tap any room to view live, localized sensor telemetry (like bedroom CO₂ levels).<br><br>
            <strong>Diagnostics:</strong> The raw data ledger. This logs the exact measurements captured by our analysts. Clicking any row will pull up the specific instrument record (like thermal camera imagery or blower door logs).
          </div>
        </div>
      `;
    }
  ...
  const injectHelpSystem = (role) => {
    ...
    const helpModalHTML = `
      <div id="jbHelpModal" class="jb-hidden">
        <div class="jb-prof-frame" style="max-height: 85vh;">
          <div class="jb-prof-header">
            <div>
              <div class="jb-prof-label" style="margin-bottom:0.25rem;">Knowledge Base</div>
              <h2 style="font-size:18px; font-weight:500; font-family:var(--font-sans, sans-serif); color:var(--foreground, #1A241D); margin:0;">${titleText}</h2>
            </div>
            <button id="closeHelpModal" class="jb-prof-close">
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:1.25rem; height:1.25rem;"><path d="M18 6 6 18"></path><path d="m6 6 12 12"></path></svg>
            </button>
          </div>
          <div class="jb-prof-body" style="padding: 1rem; gap: 0;">
            ${getHelpContent(role)}
          </div>
        </div>
      </div>
    ...
    // Accordions
    const accordions = document.querySelectorAll('.jb-accordion-item');
    accordions.forEach(acc => {
      const header = acc.querySelector('.jb-accordion-header');
      ...
      try {
        ...
        if (member && member.data) {
          ...
          if (currentUserRole === 'client' || currentUserRole === 'demo') {
            ...
            // HARD VAPORIZE: Instantly delete bottom bars for clients!
