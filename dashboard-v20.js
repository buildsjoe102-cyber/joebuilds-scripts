 ...
 * Dashboard Controller (v20 - Dynamic Modal & Subtitle Date)
 */
const JoeBuildsDashboard = (() => {
      ...
      DOM.metricCards.forEach(card => { 
        card.addEventListener('click', () => { 
          ...
          if (modalBody) {
             ...
             // Rebind close button since we overwrote the DOM
  ...
  const init = async () => {
    DOM = { 
      ...
      closeBtns: [document.getElementById('modalHeaderCloseBtn')]
    };
    ...
    try {
      ...
      if (targetBuildingId) {
        ...
        if (data.building) {
          ...
          // INJECT DYNAMIC LATEST ASSESSMENT DATE
          const descEl = document.querySelector('.jb-page-description');
          if (descEl && data.sessions && data.sessions.length > 0 && data.sessions[0].assessment_date) {
              ...
              if (!dateInsert) {
                  ...
                  descEl.parentNode.insertBefore(dateInsert, descEl.nextSibling);
              }
