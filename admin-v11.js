 ...
 * Administrative Data Controller (v11 - Full Unbroken Code + Room Context Uploader)
 */
const JoeBuildsAdmin = (() => {
  ...
  const toggleSkeletonState = (isLoading) => {
    ...
    if (isLoading && DOM.twinNodesContainer) DOM.twinNodesContainer.innerHTML = `<div class="card-surface jb-skeleton-block" style="height: 150px;"></div>`;
    if (isLoading && DOM.pathwayContainer) DOM.pathwayContainer.innerHTML = `<div class="pathway-row jb-skeleton-block" style="height: 60px;"></div>`;
  };
  ...
  const initUIEvents = () => {
    ...
    if (DOM.btnInitializeAsset) {
      DOM.btnInitializeAsset.addEventListener('click', async () => {
        DOM.btnInitializeAsset.textContent = "Deploying Baseline...";
        DOM.btnInitializeAsset.disabled = true;
        await initializeBuildingData(currentBuildingId);
      });
    }
  ...
  const authenticateAdmin = async () => {
    ...
    if (DOM.opEmail) DOM.opEmail.textContent = `Role: ${currentUserRole.toUpperCase()}`;
    
    if ((currentUserRole === 'admin' || currentUserRole === 'operator') && DOM.btnUserManagement) {
  ...
  const loadGlobalAssets = async () => {
    ...
    if (error || !buildings || buildings.length === 0) {
      if(DOM.assetCount) DOM.assetCount.textContent = `0 managed assets`;
      if(DOM.assetSelect) DOM.assetSelect.innerHTML = `<option>No properties found</option>`;
  ...
  const loadAssetContext = async (buildingId) => {
    try {
      ...
      // POPULATE UPLOADER DROPDOWN WITH DYNAMIC ROOMS
      ...
      if (mData.length === 0 && sData.length === 0) {
        if(DOM.editorsWrap) DOM.editorsWrap.classList.add('jb-hidden');
        if(DOM.emptyStateContainer) DOM.emptyStateContainer.classList.remove('jb-hidden');
      } else {
        if(DOM.emptyStateContainer) DOM.emptyStateContainer.classList.add('jb-hidden');
        if(DOM.editorsWrap) DOM.editorsWrap.classList.remove('jb-hidden');
        renderTelemetryEditor(mData);
        renderTwinEditor(rData, mData);
  ...
  const initializeBuildingData = async (bId) => {
    try {
      ...
      if(DOM.btnInitializeAsset) {
        DOM.btnInitializeAsset.textContent = "Generate Baseline Data Profile";
        DOM.btnInitializeAsset.disabled = false;
      }
  ...
  const openUsersModal = async () => {
    ...
    if(!profiles) {
      DOM.usersTableBody.innerHTML = `<tr><td colspan="4" style="text-align:center;">Failed to load users.</td></tr>`;
      return;
    }
    ...
    profiles.forEach(p => {
      ...
      tr.innerHTML = `
        ...
        <td>
          <select class="control-select" data-pid="${p.id}" data-type="building" style="padding: 0.25rem;">
            ${bOptions}
          </select>
        </td>
      `;
      ...
      if (p.building_id) {
        const bSelect = tr.querySelector(`select[data-type="building"]`);
        if(bSelect) bSelect.value = p.building_id;
      }
  ...
  const renderTelemetryEditor = (measurements) => {
    if(!DOM.telemetryContainer) return;
    ...
    DOM.telemetryContainer.querySelectorAll('input:not([disabled]), select, textarea').forEach(input => {
      input.addEventListener('change', (e) => {
        const payload = {}; payload[e.target.getAttribute('data-col')] = e.target.value;
        autoSaveData(e.target.getAttribute('data-table'), e.target.getAttribute('data-id'), payload, e.target.closest('.card-surface'));
      });
    });
  };

  const renderTwinEditor = (rooms, measurements) => {
    if(!DOM.twinNodesContainer) return;
    ...
    rooms.forEach(room => {
      ...
      const cardHTML = `
        <div class="card-surface" id="card-${room.id}">
          <div class="card-title" style="display:flex; justify-content:space-between; align-items:center;">
            ${room.room_name_current}
            <select class="control-select" style="width: auto; padding: 0.25rem;" data-table="measurements" data-id="${overallM.id}" data-col="status_flag">${generateStatusOptions(overallM.status_flag)}</select>
          </div>
          <div class="node-input-grid">
            <label><div class="eyebrow" style="margin-bottom: 0.25rem;">RH%</div>${getValInput('RH')}</label>
            <label><div class="eyebrow" style="margin-bottom: 0.25rem;">CO₂</div>${getValInput('CO2')}</label>
            <label><div class="eyebrow" style="margin-bottom: 0.25rem;">VOC</div>${getValInput('VOC')}</label>
          </div>
    ...
    DOM.twinNodesContainer.querySelectorAll('input:not([disabled]), select, textarea').forEach(input => {
      input.addEventListener('change', (e) => {
        const payload = {}; payload[e.target.getAttribute('data-col')] = e.target.value;
        autoSaveData(e.target.getAttribute('data-table'), e.target.getAttribute('data-id'), payload, e.target.closest('.card-surface'));
      });
  ...
  const renderPathwayEditor = (scenarios) => {
    if(!DOM.pathwayContainer) return;
    ...
    DOM.pathwayContainer.querySelectorAll('select').forEach(input => {
      input.addEventListener('change', (e) => {
        const payload = {}; payload[e.target.getAttribute('data-col')] = e.target.value;
        autoSaveData(e.target.getAttribute('data-table'), e.target.getAttribute('data-id'), payload, e.target.closest('.pathway-row'));
      });
    });
  };
  ...
  const handleReportUpload = async (e) => {
    ...
    // Check if the operator selected a specific room to attach this image to
    ...
    // If it's an image AND a room is selected, save to internal evidence so Digital Twin sees it.
    // Otherwise, save to reports so the client can download it.
