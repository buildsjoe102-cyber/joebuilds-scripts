 ...
 * Global Profile Manager (v32 - Client Switcher & Account Deletion)
 */
const JoeBuildsProfileManager = (() => {
  ...
  const bindClientPropertySwitcher = async (profile) => {
    ...
    if (validAccesses.length > 1) {
       // Target the specific header asset element across all pages
       const headerAssetEl = document.getElementById('desktopHeaderAsset');
       if (headerAssetEl) {
           let optionsHtml = '';
           ...
           headerAssetEl.innerHTML = `<select id="clientPropertySwitcher" style="background:transparent; border:1px solid rgba(255,255,255,0.2); border-radius:4px; color:inherit; font-family:inherit; font-size:11px; cursor:pointer; outline:none; font-weight:400; padding:2px 4px; margin-left:4px;">${optionsHtml}</select>`;
           
           document.getElementById('clientPropertySwitcher').addEventListener('change', async (e) => {
               const newBid = e.target.value;
               e.target.disabled = true;
               // Update their primary building ID in profiles so the rest of the app loads it
