 ...
 * Properties Registry Controller (v14 - Deep DOM Fallbacks & Crash Prevention)
 */
const JoeBuildsProperties = (() => {
  ...
  // Deep DOM Selectors: It will try the ID first, then fall back to the class name!
  const DOM = {
    ...
    views: { 
      registry: document.getElementById('view-registry') || document.querySelector('.content-container:nth-child(1)'), 
      newForm: document.getElementById('view-new-property') || document.querySelector('.form-container'), 
      detail: document.getElementById('view-property-detail') || document.querySelector('.content-container:nth-child(3)') 
    },
    
    sidebarAssetTitle: document.getElementById('sidebar-asset-title'), 
    headerTelemetry: document.getElementById('header-telemetry-cluster') || document.querySelector('.header-telemetry-cluster'),
    
    propertyGrid: document.getElementById('propertyGridContainer') || document.querySelector('.property-grid'), 
  ...
  const toggleSkeletonState = (isLoading) => {
    try {
      ...
      console.warn("Skeleton toggle skipped to prevent crash:", err);
    }
  };
  ...
  const openEditPropertyModal = () => {
    ...
    const codeInput = document.getElementById('edit-building-code');
    const addrInput = document.getElementById('edit-address');
    const stateInput = document.getElementById('edit-state');
    const statusInput = document.getElementById('edit-status');
    
    if(codeInput) codeInput.value = currentBuildingContext.building_code || '';
    if(addrInput) addrInput.value = currentBuildingContext.address_line_1 || '';
    if(stateInput) stateInput.value = currentBuildingContext.state || 'WA';
    if(statusInput) statusInput.value = currentBuildingContext.status || 'Baseline Pending';
  ...
  const handleSaveProperty = async () => {
    ...
    const payload = { 
      building_code: codeInput ? codeInput.value : '', 
      address_line_1: addrInput ? addrInput.value : '', 
  ...
  const openRoomModal = (id = null, code = '', name = '', x = '', y = '') => {
    ...
    const codeInput = document.getElementById('edit-room-code');
    const nameInput = document.getElementById('edit-room-name');
    const xInput = document.getElementById('edit-room-x');
    const yInput = document.getElementById('edit-room-y');
  ...
  const fetchAndRenderBuildings = async () => {
    try {
      ...
      if (!DOM.propertyGrid) {
        console.error("Critical: Could not find propertyGrid container in the DOM.");
        return;
      }

      if (!buildings || buildings.length === 0) { 
        DOM.propertyGrid.innerHTML = `<div style="padding:3rem; text-align:center; color:var(--muted-foreground); border: 1px dashed var(--border); font-size:13px;">No properties found. Please create one to initialize the registry.</div>`;
      ...
      if(DOM.propertyGrid) DOM.propertyGrid.innerHTML = `<div style="padding:2rem; color:var(--status-review);">Failed to load database.</div>`;
    } finally {
      toggleSkeletonState(false);
    }
  };
  ...
  const handleNewPropertySubmit = async () => {
    ...
    const address = document.getElementById('input-address')?.value || 'Unknown'; 
    const state = document.getElementById('input-state')?.value || 'WA'; 
    ...
    if (!error) { 
      if(DOM.formNewProperty) DOM.formNewProperty.reset(); 
      switchView('registry'); 
  ...
  const init = async () => {
    ...
    try { 
      ...
      console.error(error);
      // Removed automatic redirect to login so we can debug if needed!
    }
  };
  return { init };
})();
