 ...
 * Global Profile Manager (v18 - Universal Sidebar Text Fix)
 */
const JoeBuildsProfileManager = (() => {
  ...
  const fixTerminologyAndUI = () => {
    ...
    // Fix Desktop Nav labels (Works even if the text isn't inside a span!)
    document.querySelectorAll('.sidebar-nav a, .jb-sidebar-nav a').forEach(el => {
      const href = el.getAttribute('href');
      if (href && href.includes('/digital-twin')) {
        el.innerHTML = el.innerHTML.replace('Digital Twin', 'Zones & Rooms');
      }
    });
