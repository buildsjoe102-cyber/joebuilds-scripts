 ...
 * Global Profile Manager (v33 - Edit Modal Restored & Safe Property Switcher)
 */
const JoeBuildsProfileManager = (() => {
  ...
  const injectProfileHTML = () => {
    ...
    const modalHTML = `
      <div id="jbGlobalProfileModal" class="jb-hidden" style="position:fixed; inset:0; background:rgba(26,36,29,0.7); backdrop-filter:blur(4px); display:flex; align-items:center; justify-content:center; z-index:999999; padding:1rem;">
        <div class="jb-prof-frame" style="width:100%; max-width:28rem; background:var(--surface,#FFF); border:1px solid var(--border); box-shadow:0 20px 25px -5px rgba(0,0,0,0.1); display:flex; flex-direction:column;">
          <div class="jb-prof-header" style="display:flex; justify-content:space-between; align-items:center; padding:1.25rem 1.5rem; border-bottom:1px solid var(--border);">
            <div>
              <div class="jb-prof-label" style="font-family:var(--font-mono); font-size:9px; color:var(--muted-foreground); text-transform:uppercase; letter-spacing:0.1em; margin-bottom:0.25rem;">Account Settings</div>
              <h2 style="font-size:18px; font-weight:500; font-family:var(--font-sans); color:var(--foreground); margin:0;">Edit Profile</h2>
            </div>
            <button id="closeProfileModal" style="background:transparent; border:none; cursor:pointer; color:var(--muted-foreground); padding:0.25rem;"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 6 6 18"></path><path d="m6 6 12 12"></path></svg></button>
          </div>
          <div class="jb-prof-body" style="padding:1.5rem; display:flex; flex-direction:column; gap:1.25rem;">
            <form id="jbProfileForm" style="display:flex; flex-direction:column; gap:1.25rem; margin:0;">
              ...
              <div id="profileStatusMsg" style="font-size:12px; display:none; margin:0;"></div>
              <button type="submit" id="btnSaveProfile" class="jb-prof-btn" style="margin-top:0.5rem;">Save Changes</button>
            </form>
            <div style="border-top: 1px solid var(--border); padding-top: 1.25rem;">
               <button type="button" id="btnDeleteAccount" style="width:100%; background:transparent; border:1px solid var(--status-review); color:var(--status-review); padding:0.75rem 1rem; font-family:var(--font-mono); font-size:11px; text-transform:uppercase; letter-spacing:0.1em; cursor:pointer; transition: background 0.2s;">Delete Account</button>
            </div>
          </div>
        </div>
      </div>
  ...
  const bindProfileEvents = (profile) => {
    ...
    if(modal) {
      ...
      document.getElementById('btnDeleteAccount').addEventListener('click', async () => {
         if (confirm("Are you sure you want to permanently delete your account? This will sever your database records completely.")) {
             try {
  ...
  const bindClientPropertySwitcher = async (profile) => {
    ...
    // If they have access to multiple properties, inject the switcher into the Operator dropdown
    if (validAccesses.length > 1) {
       ...
       if (operatorMenu) {
           ...
           const switcherHtml = `
           <div style="padding: 0.75rem 1rem; border-bottom: 1px solid var(--background); background: rgba(165,179,154,0.1);">
              <div style="font-family: var(--font-mono); font-size: 9px; text-transform: uppercase; color: var(--muted-foreground); margin-bottom: 0.5rem; letter-spacing: 0.1em; font-weight: 700;">Switch Active Property</div>
              <select id="clientPropertySwitcher" style="width: 100%; border: 1px solid var(--border); background-color: var(--surface); color: var(--foreground); padding: 0.375rem 0.5rem; font-size: 11px; font-family: var(--font-mono); outline: none; cursor: pointer;">
                ${optionsHtml}
              </select>
           </div>`;
