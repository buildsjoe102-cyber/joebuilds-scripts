 ...
 * Unified Dashboard Controller (v5 - Dynamic Graphs, Global Integrity & Routing)
 */
const JoeBuildsDashboard = (() => {
  ...
  const initUIEvents = () => {
    ...
    // Connect the "Open Diagnostic Record" button to the Diagnostics Page
    ...
    DOM.metricCards.forEach(card => {
      card.addEventListener('click', () => {
        ...
        // Inject dynamically generated SVG graph
        if (DOM.modalGraph) {
  ...
  const populateDashboard = (data) => {
    ...
    // Ensure button obeys brand dictates
    ...
    // MAP INTEGRITY INDEX (Top Right Hero Badge)
