 ...
 * Properties Registry Controller (v12 - Crash-Proof, Deep Logging & Edit Modals)
 */
const JoeBuildsProperties = (() => {
  ...
  const toggleSkeletonState = (isLoading) => {
    try {
      ...
      // Only inject grid skeletons if we are in the registry view
      ...
      console.warn("Skeleton toggle skipped due to missing DOM element.");
    }
  };
  ...
  const fetchAndRenderBuildings = async () => {
    ...
    if (error || !buildings || buildings.length === 0) {
      if(DOM.propertyGrid) DOM.propertyGrid.innerHTML = `<div style="padding:2rem; color:var(--muted-foreground); font-size:13px;">No properties found. Please create one to initialize the registry.</div>`;
  ...
  const init = async () => {
    console.log("Initializing Properties Controller v12...");
    
    // Inject the skeleton CSS immediately so it's ready
    if(!document.getElementById('jb-skeleton-css')) {
    ...
    if (!window.supabase) {
      console.error("Supabase not found. Verify CDN is loaded.");
      return;
    }
