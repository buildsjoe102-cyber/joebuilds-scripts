 ...
 * Global Profile Manager (v38 - Universal Header Population)
 */
const JoeBuildsProfileManager = (() => {
  ...
  const bindProfileEvents = (profile) => {
    ...
    if (needsDemoBtn && !document.getElementById('jbTriggerDemo')) {
      ...
      demoBtn.style.borderBottom = '1px solid var(--border)';
  ...
  // --- NEW: UNIVERSAL HEADER POPULATOR ---
  // This physically forces the Header to load the correct text across all pages.
  const updateGlobalHeader = async (profile) => {
    let targetBuildingId = profile?.building_id;
    
    // Admin & Demo Fallbacks
    ...
    if (!targetBuildingId && localStorage.getItem('jb_demo_mode') === 'true') { 
        const { data: demoB } = await supabaseClient.from('buildings').select('id').eq('building_code', 'DEMO-001').single(); 
    ...
    if (targetBuildingId) {
        ...
        if (building) {
            ...
            if (headerTabs[0] && !headerTabs[0].id) { headerTabs[0].textContent = assetName; headerTabs[0].classList.remove('jb-skeleton-block'); }
            if (headerTabs[1]) { headerTabs[1].textContent = climateText; headerTabs[1].classList.remove('jb-skeleton-block'); }
            if (headerTabs[2]) { headerTabs[2].textContent = building.status || "Active Home Record"; headerTabs[2].classList.remove('jb-skeleton-block'); }
        }
    } else {
         const emptyText = "Unassigned Property";
         const headerAsset = document.getElementById('desktopHeaderAsset');
         if (headerAsset) { headerAsset.textContent = emptyText; headerAsset.classList.remove('jb-skeleton-block'); }
         const headerTabs = document.querySelectorAll('.header-telemetry-cluster .tabular, .header-telemetry-cluster .jb-tabular, .jb-desktop-telemetry .tabular, .jb-desktop-telemetry .jb-tabular');
         if (headerTabs[0]) { headerTabs[0].textContent = emptyText; headerTabs[0].classList.remove('jb-skeleton-block'); }
    }
  };
    ...
    if (window.supabase) {
      ...
      try {
        ...
        if (currentMember && currentMember.data) {
          ...
          // Populates the Header & Sidebar Universally!
