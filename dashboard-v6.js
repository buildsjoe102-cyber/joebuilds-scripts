 ...
 * Unified Dashboard Controller (v6 - Skeleton Loading State, Dynamic Graphs, Global Integrity)
 */
const JoeBuildsDashboard = (() => {
   ...
   * Injects CSS and toggles shimmer effect while Supabase fetches data
   */
  ...
  const populateDashboard = (data) => {
    ...
    // Data has populated, remove the skeleton loading animation!
  ...
  const init = async () => {
    // 1. Immediately inject CSS and turn on the Skeleton Loader!
    injectSkeletonCSS();
    ...
    try {
      ...
      if (profile?.building_id) {
        ...
        // If they have no building assigned yet, just turn off skeleton
        toggleSkeletonState(false);
      }
    } catch (error) {
      console.error(error);
