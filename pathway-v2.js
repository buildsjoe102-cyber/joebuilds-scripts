 ...
 * Pathway Controller (v2 - Modal Interactions & Routing)
 */
const JoeBuildsPathway = (() => {
  ...
  const toggleSkeletonState = (isLoading) => {
    const elementsToToggle = [DOM.desktopSidebarAsset, DOM.desktopHeaderAsset];
  ...
  const populatePathwayUI = (data) => {
    ...
    if (DOM.sequenceStack) {
      ...
      data.scenarios.forEach(scen => {
        scenariosStore[scen.id] = scen; // Store in memory for Modal
        DOM.sequenceStack.insertAdjacentHTML('beforeend', renderScenarioRow(scen));
      });
    }

    // Attach click listeners to the buttons we just injected
  ...
  const openModal = (id) => {
    ...
    // Configure the Action Button Routing
