 ...
 * Properties Registry Controller (v20 - Cache Buster & All AU States)
 */
console.log("JB Properties Controller v20 initializing (Cache Busted!)...");
...
const JoeBuildsProperties = (() => {
  ...
  const handleSaveProperty = async () => {
    ...
    if(!error) { 
      ...
      supabase.from('buildings').select('*').then(({data}) => {
        if(data) {
          // Pass the AU_STATES constant directly to the builder
  ...
  const fetchAndRenderBuildings = async () => {
    ...
    try {
      ...
      // Dynamic Datalist Population with expanded Australian States
