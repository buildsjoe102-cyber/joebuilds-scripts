 ...
 * Properties Registry Controller (v17 - Bulletproof DOM Loading & Error Handling)
 */
const JoeBuildsProperties = (() => {
  ...
  // We declare DOM here, but we wait to populate it until the page is fully loaded
  let DOM = {};
  ...
  const fetchAndRenderBuildings = async () => {
    ...
    try {
      ...
      console.error(err);
      if(DOM.propertyGrid) DOM.propertyGrid.innerHTML = `<div style="padding:3rem; text-align:center; color:var(--status-review); border: 1px dashed var(--border); font-size:13px;">Error loading properties.</div>`;
    }
    toggleSkeletonState(false);
  };
  ...
  const init = async () => {
    // 1. Wait to populate DOM references until page has fully loaded to prevent null errors
