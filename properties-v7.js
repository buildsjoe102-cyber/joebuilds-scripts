 ...
 * Properties Registry Controller (v7 - v3 Logic + Skeleton Loading Effects)
 */
const JoeBuildsProperties = (() => {
  ...
  const fetchAndRenderBuildings = async () => {
    toggleSkeletonState(true); // START SHIMMER
    ...
    toggleSkeletonState(false); // REMOVE SHIMMER
  };
  ...
  const init = async () => {
    injectSkeletonCSS(); // Inject loader CSS
    bindUIEvents();
