 ...
 * Properties Registry Controller (v15 - Interactive Map & Click-to-Pin)
 */
const JoeBuildsProperties = (() => {
  ...
  let currentBuildingRooms = []; // Cache rooms so we can draw them on the map
  ...
  const DOM = {
    ...
    sidebarAssetTitle: document.getElementById('sidebar-asset-title'), headerTelemetry: document.getElementById('header-telemetry-cluster'),
    propertyGrid: document.getElementById('propertyGridContainer'), formNewProperty: document.getElementById('form-new-property'), btnSubmitProperty: document.getElementById('btn-submit-property'),
  ...
  const handleSaveProperty = async () => {
    ...
    if(!error) { currentBuildingContext = { ...currentBuildingContext, ...payload }; openPropertyDetail(currentBuildingContext); }
  };
  ...
  const bindMapClick = () => {
    ...
    DOM.mapContainer.addEventListener('click', (e) => {
      // Calculate percentages based on click relative to the container
      ...
      // Draw or move the active green pin
      let activePin = document.getElementById('modal-active-pin');
      if (!activePin) {
        activePin = document.createElement('div');
        activePin.id = 'modal-active-pin';
        activePin.className = 'modal-map-pin active';
  ...
  const openRoomModal = (id = null, code = '', name = '', x = '', y = '') => {
    ...
    if (DOM.mapCoordsText) {
      DOM.mapCoordsText.textContent = (x && y) ? `Node set at: X ${x}%, Y ${y}%` : "Click map to set location";
    }

    // DRAW THE PINS
    if (DOM.mapPins) {
      DOM.mapPins.innerHTML = ''; // Clear old pins

      // Draw existing faded pins for other rooms
      currentBuildingRooms.forEach(r => {
        if (r.id === id) return; // Skip the room we are currently editing
        if (r.map_x != null && r.map_y != null) {
          const pin = document.createElement('div');
          pin.className = 'modal-map-pin existing';
          pin.style.left = `${r.map_x}%`;
          pin.style.top = `${r.map_y}%`;
          pin.title = r.room_name_current;
          DOM.mapPins.appendChild(pin);
        }
      });

      // Draw the active pin if editing a room that already has coordinates
      if (x && y) {
        const activePin = document.createElement('div');
        activePin.id = 'modal-active-pin';
        activePin.className = 'modal-map-pin active';
        activePin.style.left = `${x}%`;
        activePin.style.top = `${y}%`;
  ...
  const bindUIEvents = () => {
    ...
    bindMapClick(); // Bind the interactive map click
  };
  ...
  const fetchBuildingDetails = async (buildingId) => {
    ...
    if (DOM.detailRoomsList) {
      currentBuildingRooms = roomsRes.data || []; // Cache rooms for the modal map!
  ...
  const handleFileUpload = async (e) => {
    ...
    fetchBuildingAssets(currentBuildingContext.id);
  };
