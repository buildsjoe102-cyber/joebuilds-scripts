 ...
 * Dashboard Controller (v19 - Dynamic Text Overrides)
 */
const JoeBuildsDashboard = (() => {
  ...
  const mapMeasurementToCard = (data, elementCode, cardTitle, prefixText = '') => {
    ...
    if (m) {
      ...
      // FIX: Rely cleanly on the database value/unit instead of concatenating awkwardly
  ...
  const mapHistoricalLog = (sessions) => {
    ...
    if (descEl) {
        if (count > 0 && sessions[0].assessment_date) {
            const d = new Date(sessions[0].assessment_date).toLocaleDateString('en-GB');
            descEl.textContent = `Last entry: ${d}.`;
        } else {
            descEl.textContent = 'No historical sessions logged.';
        }
    }
  };
  ...
  const mapReadiness = (scenarios) => {
    ...
    if (descEl) descEl.textContent = active ? `Currently active: ${active.title}` : 'All pathways completed.';
