/**
 * Joe Builds - Admin Controller (v26 - Sidebar Alignment & Stability)
 */
console.log("JB Admin Controller v26 initializing...");
const JoeBuildsAdmin = (() => {
  ...
  const toggleSkeletonState = (isLoading) => { 
    try {
      const elementsToToggle = [DOM.desktopSidebarAsset, DOM.desktopHeaderAsset, DOM.headerClimateText, DOM.headerSysText, DOM.desktopSidebarMeta, DOM.assetCount, DOM.archiveCount]; 
  ...
  const loadAssetContext = async (buildingId) => {
    try {
      ...
      if (bData) {
        ...
        // This clears "Authenticating" on the sidebar
  ...
  const init = async () => {
    // DOM mapping delayed until init to ensure page is completely rendered
