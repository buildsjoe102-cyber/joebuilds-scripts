 ...
 * Unified Dashboard Controller (v9 - Dynamic Sidebar Meta Fix)
 */
const JoeBuildsDashboard = (() => {
  ...
  const DOM = {
    ...
    // NEW: Smart Selector for Sidebar Meta (Climate/Authenticating text)
  ...
  const injectSkeletonCSS = () => {
    ...
    style.innerHTML = `
      .jb-skeleton-block { position: relative; overflow: hidden; background-color: rgba(165, 179, 154, 0.2) !important; color: transparent !important; border-color: transparent !important; border-radius: 4px; pointer-events: none; }
  ...
  const renderEmptyState = () => {
    ...
    if (DOM.matrixContainer) {
      ...
      DOM.matrixContainer.style.background = 'transparent';
      DOM.matrixContainer.style.border = 'none';
      DOM.matrixContainer.innerHTML = `<div style="text-align:center; padding: 6rem 2rem; background: var(--surface); border: 1px dashed var(--border); margin-bottom: 3rem;"><svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1" stroke-linecap="round" stroke-linejoin="round" style="color:var(--muted-foreground); margin-bottom: 1rem;"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"></path><polyline points="9 22 9 12 15 12 15 22"></polyline></svg><h3 style="font-size: 20px; font-weight: 500; margin-bottom: 0.5rem; color: var(--foreground);">Dashboard Empty</h3><p style="color: var(--muted-foreground); font-size: 14px; max-width: 400px; margin: 0 auto;">Your dashboard will be populated with live telemetry and diagnostic data once an administrator assigns a property to your account.</p></div>`;
    }
  };

  const generateSparkline = (status) => {
    let points = []; let currentY = 32; 
    for(let i=0; i<12; i++) {
       let jitter = (status === 'risk' || status === 'review') ? (Math.random() * 30 - 15) : (Math.random() * 10 - 5);
       currentY = Math.max(5, Math.min(59, currentY + jitter)); points.push(currentY);
    }
    const stepX = 300 / 11;
    ...
    return `<svg viewBox="0 0 300 64" preserveAspectRatio="none" style="width:100%;height:6rem;display:block;color:var(--foreground);"><defs><pattern id="dgrid" width="25" height="16" patternUnits="userSpaceOnUse"><path d="M 25 0 L 0 0 0 16" fill="none" stroke="currentColor" stroke-opacity="0.08" stroke-width="0.5"></path></pattern></defs><rect width="300" height="64" fill="url(#dgrid)"></rect><path d="${pathD}" fill="none" stroke="currentColor" stroke-width="1.2"></path>${circles}</svg>`;
  };
  ...
  const initUIEvents = () => {
    ...
    DOM.metricCards.forEach(card => {
      card.addEventListener('click', () => {
        ...
        DOM.mTitle.textContent = card.getAttribute('data-title'); DOM.mStatusText.textContent = card.getAttribute('data-badge');
        DOM.mCurrent.textContent = card.getAttribute('data-current'); DOM.mDelta7.textContent = card.getAttribute('data-delta7');
        DOM.mDelta30.textContent = card.getAttribute('data-delta30'); DOM.mThreshold.textContent = card.getAttribute('data-threshold');
        DOM.mCommentary.textContent = card.getAttribute('data-commentary'); DOM.mRec.textContent = card.getAttribute('data-rec');
        DOM.mStatusBadge.className = `jb-status-badge jb-status-${status}`; DOM.mStatusDot.className = `jb-badge-dot bg-${status}`;
        if (DOM.modalGraph) DOM.modalGraph.innerHTML = generateSparkline(status);
  ...
  const getBadgeText = (status) => {
    if(status === 'risk') return 'At Risk'; if(status === 'review') return 'Review Required'; if(!status) return 'Unknown';
  ...
  const mapMeasurementToCard = (data, elementCode, cardTitle, prefixText = '') => {
    ...
    if (m) {
      updateCardAttributes(cardTitle, {
        status: m.status_flag || 'unknown', badge: getBadgeText(m.status_flag),
        current: `${prefixText} ${m.value || ''} ${m.unit || ''}`.trim(),
        commentary: m.client_facing_wording || 'System tracking active.', rec: "Consult pathway for recommended next actions." 
      });
    }
  };
  ...
  const populateDashboard = (data) => {
    ...
    // Dynamic Climate Text
    if (DOM.sidebarMeta) {
      const state = data.building.state || 'WA';
      DOM.sidebarMeta.textContent = state === 'WA' ? 'Zone 5 — Warm Temperate' : 'Zone 6 — Mild Temperate';
    }
    ...
    if (data.scenarios && DOM.ctaTitle) {
      ...
      if (nextAction) DOM.ctaTitle.textContent = nextAction.title.replace(/Forensic/g, "Diagnostic");
  ...
  const updateCardAttributes = (title, mappedData) => {
    ...
    if (!card) return;
    card.setAttribute('data-status', mappedData.status); card.setAttribute('data-badge', mappedData.badge);
    card.setAttribute('data-current', mappedData.current); card.setAttribute('data-commentary', mappedData.commentary);
    ...
    if (valueEl) valueEl.textContent = mappedData.current; if (descEl) descEl.textContent = mappedData.commentary;
