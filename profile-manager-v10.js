 ...
 * Global Profile Manager (v10 - Ultra-Fast, No CSS Injection FOUC)
 */
  ...
  const bindProfileEvents = () => {
    ...
    document.getElementById('jbProfileForm').addEventListener('submit', async (e) => {
      ...
      try {
        ...
        if (currentMember && currentMember.data) await supabaseClient.from('profiles').update({ first_name: newName, email: newEmail }).eq('memberstack_id', currentMember.data.id);
