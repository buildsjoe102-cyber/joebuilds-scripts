 ...
 * Digital Twin Controller (v7 - SVG Restored, Dynamic Mapping & Slide-over Modal)
 */
const JoeBuildsDigitalTwin = (() => {
  ...
  // Library of Coordinates for the SVG Map
  const roomCoordinates = {
    'primary': { left: '22%', top: '28%' },
    'kitchen': { left: '60%', top: '32%' },
    'plant': { left: '78%', top: '62%' },
    'subfloor': { left: '40%', top: '78%' },
    'living': { left: '45%', top: '50%' },
    'bathroom': { left: '80%', top: '20%' }, // Mapped your new room!
    'util': { left: '15%', top: '60%' },
    'deck': { left: '60%', top: '80%' }
  };
  ...
  const DOM = {
    ...
    dynamicHotspots: document.getElementById('dynamic-hotspots'),
    
    // Aside Modal (Now sliding overlay)
    aside: document.getElementById('jbDiagnosticAside'),
    backdrop: document.getElementById('jbAsideBackdrop'),
  ...
  const initUIEvents = () => {
    ...
    // Slide-over Modal Close Handlers
    const closeAside = () => {
  ...
  const renderDashboard = (data) => {
    ...
    // MAP HOTSPOTS DYNAMICALLY TO THE SVG BASE
    if (DOM.dynamicHotspots) {
      DOM.dynamicHotspots.innerHTML = '';
      let unknownOffset = 10; // Fallback positioning X coordinate

      zoneKeys.forEach(key => {
        const zone = activeZonesData[key];
        let coords = roomCoordinates[zone.code];
        
        if (!coords) {
          // Fallback if room code isn't in library (draws them along the bottom)
          coords = { left: `${unknownOffset}%`, top: '90%' };
          unknownOffset += 15;
        }

        const hotspotHTML = `
          <button class="jb-hotspot" style="left:${coords.left}; top:${coords.top};" data-id="${key}">
            <span class="jb-hotspot-pulse">
              <span class="jb-hotspot-ping-wave bg-${zone.status}"></span>
              <span class="jb-hotspot-core-ring" style="border-color: var(--status-${zone.status});"></span>
            </span>
            <span class="jb-hotspot-tag">${zone.title}</span>
          </button>
        `;
      ...
      // Bind Clicks
      document.querySelectorAll('.jb-hotspot').forEach(node => {
  ...
  const openDiagnosticPanel = (zoneId) => {
    ...
    const tNode = document.querySelector(`.jb-hotspot[data-id="${zoneId}"]`);
