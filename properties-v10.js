 ...
 * Properties Registry Controller (v10 - Wildcard Link Reveal Fix)
 */
const JoeBuildsProperties = (() => {
  ...
  const fetchAndRenderBuildings = async () => {
    ...
    if (!buildings) { toggleSkeletonState(false); return; }
  ...
  const handleNewPropertySubmit = async () => {
    ...
    const bCode = document.getElementById('input-building-code')?.value || 'PRJ'; const address = document.getElementById('input-address')?.value || 'Unknown'; const status = document.getElementById('input-status')?.value || 'Baseline Pending';
  ...
  const openPropertyDetail = async (building) => {
    ...
    await fetchBuildingAssets(building.id); toggleSkeletonState(false);
  };
  ...
  const handleFileUpload = async (e) => {
    ...
    const fileName = `${currentBuildingContext.id}-${Date.now()}.${file.name.split('.').pop()}`;
