 ...
 * Diagnostics Controller (v10 - Language Recalibration & Demo Mode)
 */
const JoeBuildsDiagnostics = (() => {
  ...
  const DOM = { 
    desktopSidebarAsset: document.getElementById('desktopSidebarAsset'), desktopHeaderAsset: document.getElementById('desktopHeaderAsset'), headerClimateText: document.querySelectorAll('.jb-desktop-telemetry .jb-tabular')[1], headerSysText: document.querySelectorAll('.jb-desktop-telemetry .jb-tabular')[2], sidebarMeta: document.getElementById('desktopSidebarMeta'), 
    tbodyThermal: document.getElementById('tbody-thermal'), mgridThermal: document.getElementById('mgrid-thermal'), countThermal: document.getElementById('count-thermal'), tbodyAir: document.getElementById('tbody-air'), mgridAir: document.getElementById('mgrid-air'), countAir: document.getElementById('count-air'), 
    modal: document.getElementById('jbDiagnosticModal'), modalCloseBtn: document.getElementById('modalCloseBtn'), mTitle: document.getElementById('modalMetricTitle'), mCapture: document.getElementById('modalCapture'), mOp: document.getElementById('modalOperator'), mInst: document.getElementById('modalInstrument'), mNotes: document.getElementById('modalNotes'), modalGraphicWrapper: document.querySelector('#jbDiagnosticModal > div > div:nth-child(2) > div:nth-child(1)') 
  };
  ...
  const toggleSkeletonState = (isLoading) => { 
    ...
    if (isLoading && DOM.tbodyThermal) { DOM.tbodyThermal.innerHTML = `<tr><td colspan="5"><div class="jb-skeleton-block" style="height: 40px;"></div></td></tr>`; } 
  };
  ...
  const mapToDOM = (data) => {
    ...
    if (DOM.tbodyThermal && DOM.countThermal) { 
      ...
      thermals.forEach(m => { 
        const title = m.measurement_points.element_code === 'U-VALUE' ? 'Assembly U-Value' : m.measurement_points.element_code === 'ENVELOPE' ? 'Envelope Integrity' : 'Metric'; 
        DOM.tbodyThermal.insertAdjacentHTML('beforeend', `<tr data-id="${m.id}" style="cursor: pointer;"><td><button class="jb-img-btn"><svg style="width:14px;height:14px"><use href="#i-img"/></svg></button></td><td>${title}</td><td class="jb-font-mono jb-tabular">${m.value || '-'}</td><td><span class="jb-status-badge jb-status-${m.status_flag}">${m.status_flag}</span></td><td style="color:var(--muted-foreground)">${m.client_facing_wording || '-'}</td></tr>`); 
      }); 
    }
    ...
    if (DOM.tbodyAir && DOM.countAir) { 
      ...
      airTights.forEach(m => { 
        const title = m.measurement_points.element_code === 'ACH50' ? 'ACH50 Air Infiltration Rate' : m.measurement_points.element_code === 'CO2' ? 'Indoor Air Quality' : 'Metric'; 
        DOM.tbodyAir.insertAdjacentHTML('beforeend', `<tr data-id="${m.id}" style="cursor: pointer;"><td><button class="jb-img-btn"><svg style="width:14px;height:14px"><use href="#i-img"/></svg></button></td><td>${title}</td><td class="jb-font-mono jb-tabular">${m.value || '-'}</td><td><span class="jb-status-badge jb-status-${m.status_flag}">${m.status_flag}</span></td><td style="color:var(--muted-foreground)">${m.client_facing_wording || '-'}</td></tr>`); 
      }); 
    }

    document.querySelectorAll('tr[data-id]').forEach(el => { 
      el.addEventListener('click', (e) => { 
        e.stopPropagation(); const metricName = el.querySelector('td:nth-child(2)')?.textContent; 
        DOM.mTitle.textContent = metricName; DOM.modal.classList.remove('jb-hidden'); 
      }); 
    });
  ...
  const init = async () => {
    ...
    const eyebrow = document.querySelector('.jb-eyebrow');
    if (eyebrow && eyebrow.textContent.includes('Diagnostic Log')) eyebrow.textContent = 'Measurement Log / 03';
