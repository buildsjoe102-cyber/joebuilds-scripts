 ...
 * Digital Twin Controller (v6 - Procedural Spatial Matrix, Custom Notes, Room Imagery)
 */
const JoeBuildsDigitalTwin = (() => {
  ...
  const DOM = {
    ...
    spatialMatrix: document.getElementById('dynamic-spatial-matrix'),
  ...
  const toggleSkeletonState = (isLoading) => {
    ...
    if (isLoading && DOM.spatialMatrix) DOM.spatialMatrix.innerHTML = `<div class="jb-spatial-node jb-skeleton-block" style="border:none;"></div><div class="jb-spatial-node jb-skeleton-block" style="border:none;"></div>`;
  };
  ...
  const initUIEvents = () => {
    ...
    const closeAside = () => {
      ...
      document.querySelectorAll('.jb-spatial-node').forEach(el => el.classList.remove('active'));
  ...
  const processZoneData = (rooms, measurements) => {
    ...
    rooms.forEach(room => {
      ...
      // Look for active sensors to generate dynamic monitoring text
      const activeSensors = [];
      ...
      zones[room.id] = { // Using room.id as key for accuracy
        id: room.id,
        ...
        notes: room.notes || 'No active analyst notes for this zone.', // Pulls directly from room table!
  ...
  const renderDashboard = (data) => {
    ...
    // PROCEDURAL SPATIAL MATRIX GENERATION
    if (DOM.spatialMatrix) {
      DOM.spatialMatrix.innerHTML = '';
      zoneKeys.forEach(key => {
        const zone = activeZonesData[key];
        // Create an abstract block for every room in the database
        const nodeHTML = `
          <div class="jb-spatial-node" data-id="${key}">
            <div class="jb-node-label">${zone.code}</div>
            <div class="jb-hotspot-pulse">
              <span class="jb-hotspot-ping-wave" style="background-color: var(--status-${zone.status});"></span>
              <span class="jb-hotspot-core-ring" style="border-color: var(--status-${zone.status});"></span>
            </div>
          </div>
        `;
        DOM.spatialMatrix.insertAdjacentHTML('beforeend', nodeHTML);
      });

      // Bind Clicks to new procedural nodes
      document.querySelectorAll('.jb-spatial-node').forEach(node => {
        node.addEventListener('click', (e) => {
          e.stopPropagation();
          openDiagnosticPanel(node.getAttribute('data-id'));
        });
      });
    }
  ...
  const openDiagnosticPanel = (zoneId) => {
    ...
    if (DOM.imageryGrid) {
      // Find images uploaded to this specific room_id
      const roomImages = globalImages.filter(img => img.room_id === zoneId);
    ...
    const tNode = document.querySelector(`.jb-spatial-node[data-id="${zoneId}"]`);
    const tRow = document.querySelector(`.jb-zone-row-item[data-id="${zoneId}"]`);
