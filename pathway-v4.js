 ...
 * Pathway Controller (v4 - Dynamic Sidebar Meta)
 */
const JoeBuildsPathway = (() => {
  ...
  const toggleSkeletonState = (isLoading) => {
    ...
    if (isLoading && DOM.sequenceStack) DOM.sequenceStack.innerHTML = `<div class="sequence-row"><div class="jb-skeleton-block" style="height: 60px; width: 100%;"></div></div><div class="sequence-row"><div class="jb-skeleton-block" style="height: 60px; width: 100%;"></div></div><div class="sequence-row"><div class="jb-skeleton-block" style="height: 60px; width: 100%;"></div></div>`;
  };
  ...
  const initUIEvents = () => {
    ...
    if (DOM.modalCloseBtn) DOM.modalCloseBtn.addEventListener('click', closeModal); if (DOM.modalCancelBtn) DOM.modalCancelBtn.addEventListener('click', closeModal);
  ...
  const renderScenarioRow = (scenario) => {
    const safeTitle = scenario.title.replace(/Forensic/g, "Diagnostic"); const safeDesc = (scenario.client_facing_wording || '').replace(/Forensic/g, "Diagnostic");
  ...
  const updatePhaseUI = (phaseNum, elements, scenarios) => {
    const phaseScenarios = scenarios.filter(s => s.phase === phaseNum); const total = phaseScenarios.length; if (total === 0) return;
    const completed = phaseScenarios.filter(s => s.status === 'completed').length; const progressPerc = Math.round((completed / total) * 100);
    if (elements.count) elements.count.textContent = `${completed}/${total}`; if (elements.bar) elements.bar.style.width = `${progressPerc}%`; if (elements.icon && progressPerc === 100) elements.icon.style.color = 'var(--accent)';
  };
  ...
  const populatePathwayUI = (data) => {
    ...
    document.querySelectorAll('.action-btn:not([disabled])').forEach(btn => { btn.addEventListener('click', (e) => { openModal(e.target.getAttribute('data-id')); }); });
    updatePhaseUI(1, { count: DOM.p1Count, bar: DOM.p1Bar, icon: DOM.p1Icon }, data.scenarios); updatePhaseUI(2, { count: DOM.p2Count, bar: DOM.p2Bar, icon: DOM.p2Icon }, data.scenarios); updatePhaseUI(3, { count: DOM.p3Count, bar: DOM.p3Bar }, data.scenarios);
    toggleSkeletonState(false);
  };

  const openModal = (id) => {
    const scen = scenariosStore[id]; if(!scen) return;
    DOM.mPhaseName.textContent = `Phase 0${scen.phase} · ${scen.phase_name}`; DOM.mTitle.textContent = scen.title.replace(/Forensic/g, "Diagnostic"); DOM.mDesc.textContent = (scen.client_facing_wording || '').replace(/Forensic/g, "Diagnostic");
    const statusClass = scen.status === 'completed' ? 'stable' : 'measured'; const statusText = scen.status === 'completed' ? 'Completed' : 'In Progress';
    DOM.mStatusBadge.className = `status-badge ${statusClass}`; DOM.mStatusIcon.innerHTML = getStatusIcon(scen.status); DOM.mStatusText.textContent = statusText;
    if(scen.dependency_text) { DOM.mDependencyText.textContent = scen.dependency_text; DOM.mDependencyWrap.classList.remove('jb-hidden'); } else { DOM.mDependencyWrap.classList.add('jb-hidden'); }
    if (scen.status === 'completed') { DOM.mActionBtn.textContent = 'Go to Reports Center'; DOM.mActionBtn.onclick = () => window.location.href = '/reports'; } else { DOM.mActionBtn.textContent = 'View Live Telemetry'; DOM.mActionBtn.onclick = () => window.location.href = '/diagnostics'; }
  ...
  const init = async () => {
    ...
    try { const profile = await authAndGetProfile(); if (profile?.building_id) { const pathwayData = await fetchPathwayData(profile.building_id); populatePathwayUI(pathwayData); } else { toggleSkeletonState(false); } } catch (error) { window.location.href = '/login'; }
  };
  return { init };
})();
