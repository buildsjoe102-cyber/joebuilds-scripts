/**
 * Joe Builds - Properties Registry Controller (v26 - Datalist UX Fix & Delete Room)
 */
console.log("JB Properties Controller v26 initializing...");
const JoeBuildsProperties = (() => {
  ...
  // UX FIX: Temporarily clear input on focus so all datalist options show, restore on blur if empty
  const applyComboboxUX = (inputElement) => {
    ...
    inputElement.addEventListener('blur', function() {
      if (this.value.trim() === '') this.value = this.getAttribute('data-old-val') || '';
    });
  };
  ...
  const fixEditModalDropdowns = () => {
    ...
    // Apply UX fix to the Create form as well
  ...
  const openRoomModal = (id = null, code = '', name = '', x = '', y = '') => {
    ...
    if (id) {
      ...
      if (delBtn) delBtn.style.display = 'none'; // Hide if adding a new room
    }
  ...
  const fetchAndRenderBuildings = async () => {
    ...
    } catch (err) { if(DOM.propertyGrid) DOM.propertyGrid.innerHTML = `<div style="padding:3rem; text-align:center; color:var(--status-review); border: 1px dashed var(--border); font-size:13px;">Error loading properties: ${err.message}</div>`; }
