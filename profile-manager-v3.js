 ...
 * Global Profile Manager (v3 - Profile Editing + Smart Mobile Hamburger Menu)
 */
const JoeBuildsProfileManager = (() => {
  ...
  const injectHamburgerCSS = () => {
    ...
    style.innerHTML = `
      ...
      /* The Killswitch: Vaporizes the bottom bar if hamburger is active */
  ...
  const setupHamburgerMenu = () => {
    ...
    document.body.classList.add('use-hamburger'); // Hides bottom bar globally

    const headerContainer = document.querySelector('.header-container');
    const mobileBrand = document.querySelector('.mobile-brand');
    
    if (headerContainer && mobileBrand) {
      // 1. Group the Hamburger and Logo to the left
  ...
  const init = () => {
    // Escape hatch: Only run if user is inside the portal
    ...
    if (window.supabase) {
      ...
      // Initialize Smart Hamburger Menu (Mobile Only)
      if (window.innerWidth <= 768) {
        injectHamburgerCSS();
        
        // Use a MutationObserver to wait until the page's main controller finishes loading data
        const observer = new MutationObserver(() => {
          if (!document.querySelector('.jb-skeleton-block')) {
            // Data has loaded. Check if they are a Client.
            // If `#rbac-hide-admin` exists, it means the user is a Client/Demo.
            if (document.getElementById('rbac-hide-admin')) {
              setupHamburgerMenu();
            }
