/**
 * Joe Builds - Admin Controller (v33 - Clean Modals & 406/403 Error Fix)
 */
console.log("JB Admin Controller v33 initializing...");
const JoeBuildsAdmin = (() => {
  ...
  const initUIEvents = () => {
    ...
    if (DOM.btnAccessAssign) {
      DOM.btnAccessAssign.addEventListener('click', async () => {
         ...
         // 1. Assign to User Property Access Table (.maybeSingle avoids the 406 Error)
  ...
  // NEW: THE SECONDARY ACCESS MODAL LOGIC
       ...
       if (!accesses || accesses.length === 0) {
           ...
           DOM.accessModalList.querySelectorAll('.btn-remove-access').forEach(btn => {
               btn.addEventListener('click', async () => {
                  ...
                  // If we deleted their primary profile building_id, clear it
