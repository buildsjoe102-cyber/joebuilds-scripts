/**
 * Joe Builds - Admin Controller (v27 - Expanded Assessment & Issue Fields)
 */
console.log("JB Admin Controller v27 initializing...");
const JoeBuildsAdmin = (() => {
  ...
  // --- UPDATED RENDER ASSESSMENTS WITH DATE AND NOTES ---
    ...
    sessions.forEach(s => {
      ...
      <div class="card-surface" id="sess-${s.assessment_id}">
        ...
        <div class="metrics-input-grid">
          ...
          <input type="date" class="control-input" value="${dateVal}" data-table="assessment_sessions" data-id="${s.assessment_id}" data-col="assessment_date" title="Assessment Date">
        </div>
        <textarea rows="2" class="control-textarea" placeholder="Baseline notes / Weather conditions..." data-table="assessment_sessions" data-id="${s.assessment_id}" data-col="baseline_notes">${s.baseline_notes || ''}</textarea>
      </div>`;
  ...
  // --- UPDATED RENDER ISSUES WITH SEVERITY AND ACTION ---
      ...
      <div class="card-surface" id="issue-${i.issue_id}">
        ...
        <div class="metrics-input-grid">
          ...
          <select class="control-select" data-table="issues_findings" data-id="${i.issue_id}" data-col="status">
             <option value="Stable" ${i.status === 'Stable' ? 'selected' : ''}>Stable</option>
             ...
             <option value="Verification Required" ${i.status === 'Verification Required' ? 'selected' : ''}>Verification Required</option>
             <option value="Closed" ${i.status === 'Closed' ? 'selected' : ''}>Closed</option>
          </select>
        </div>
        <div class="metrics-input-grid">
          <select class="control-select" data-table="issues_findings" data-id="${i.issue_id}" data-col="severity">
             <option value="" ${!i.severity ? 'selected' : ''}>Select Severity...</option>
        ...
        <input class="control-input" placeholder="Recommended Action" value="${i.recommended_action || ''}" data-table="issues_findings" data-id="${i.issue_id}" data-col="recommended_action">
