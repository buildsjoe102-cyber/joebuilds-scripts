 ...
 * Global Profile Manager (v24 - Nav & Sidebar Reordering)
 */
const JoeBuildsProfileManager = (() => {
  ...
  const injectGlobalCSS = () => {
    ...
    style.innerHTML = `
      ...
      .jb-tabular, .jb-metric-string, .jb-integrity-value, .jb-status-badge, .header-telemetry-cluster { opacity: 0; }
      body.jb-data-ready .jb-tabular, body.jb-data-ready .jb-metric-string, body.jb-data-ready .jb-integrity-value, body.jb-data-ready .jb-status-badge, body.jb-data-ready .header-telemetry-cluster { opacity: 1 !important; transition: opacity 0.4s ease; }
    `;
  ...
  const fixTerminologyAndUI = () => {
    ...
    // 5. DOM Reordering: Swap Reports and Properties in Desktop Sidebar
    document.querySelectorAll('.sidebar-nav, .jb-sidebar-nav').forEach(nav => {
      ...
      // If both exist, move Reports to be immediately before Properties
      if (propLink && repLink) {
        nav.insertBefore(repLink, propLink);
      }
    });

    // 6. Remove old mobile navs
    document.querySelectorAll('.mobile-nav, .jb-bottom-bar, .jb-mobile-overlay, .jb-hamburger-btn').forEach(el => el.remove());

    // 7. Inject Recalibrated Bottom Nav (REORDERED: Reports before Properties)
