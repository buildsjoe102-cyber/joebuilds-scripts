 ...
 * Global Profile Manager (v17 - Exclude Public Pages Fix)
 */
const JoeBuildsProfileManager = (() => {
  ...
  const fixTerminologyAndUI = () => {
    ...
    // Replace "ASSET" with "PROPERTY" in all headers
    ...
    // Replace Sidebar "Asset" with "Measured Home"
    document.querySelectorAll('.sidebar-footer .eyebrow').forEach(el => {
      if(el.textContent.includes('Asset')) el.textContent = 'Measured Home';
    });

    // Fix Desktop Nav labels
    document.querySelectorAll('.sidebar-nav a span, .jb-sidebar-nav a span').forEach(el => {
      if(el.textContent.includes('Digital Twin')) el.textContent = 'Zones & Rooms';
    });

    // Remove old mobile navs
    document.querySelectorAll('.mobile-nav, .jb-bottom-bar, .jb-mobile-overlay, .jb-hamburger-btn').forEach(el => el.remove());

    // Inject Recalibrated Bottom Nav
  ...
  const init = async () => {
    // 1. EXIT IMMEDIATELY IF WE ARE NOT IN THE APP (e.g., Homepage or Login page)
