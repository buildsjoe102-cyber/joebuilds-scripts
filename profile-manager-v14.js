 ...
 * Global Profile Manager (v13 - Updated Knowledge Base & Master Layout)
 */
const JoeBuildsProfileManager = (() => {
  ...
  const fixMobileNavText = () => {
    document.querySelectorAll('.mobile-nav-btn, .jb-bottom-item').forEach(btn => {
      ...
      if (href === '/pathway' || href === '/digital-twin') {
        ...
        if (svg) { btn.innerHTML = ''; btn.appendChild(svg); btn.appendChild(document.createTextNode(newText)); }
      }
    });
  };
  ...
  // ==========================================
  // MODULE 3: ROLE-BASED HELP SYSTEM (UPDATED)
  // ==========================================
  const getHelpContent = (role) => {
    if (role === 'admin' || role === 'operator') {
      ...
      return `
        ...
        <div class="jb-accordion-item"><button class="jb-accordion-header">Profile & Mobile Navigation <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m6 9 6 6 6-6"></path></svg></button><div class="jb-accordion-content"><strong>Mobile Users:</strong> Use the slide-out menu (top left) to navigate between your Dashboard, Twin, and Reports.<br><br><strong>Account Management:</strong> Click the <strong>Operator Dropdown</strong> in the top right to access <strong>Edit Profile</strong>. You can update your information or delete your account to remove your data from our systems for privacy compliance.</div></div>
      `;
    }
  };
