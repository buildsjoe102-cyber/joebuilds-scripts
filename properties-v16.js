 ...
 * Properties Registry Controller (v16 - Shimmer Skeleton Only)
 */
const JoeBuildsProperties = (() => {
  ...
  const DOM = {
    ...
    propertyGrid: document.getElementById('propertyGridContainer') || document.querySelector('.property-grid'), formNewProperty: document.getElementById('form-new-property'), btnSubmitProperty: document.getElementById('btn-submit-property'),
  ...
  const toggleSkeletonState = (isLoading) => {
    try {
      ...
      if (isLoading && DOM.propertyGrid && DOM.views.registry && !DOM.views.registry.classList.contains('jb-hidden')) {
  ...
  const fetchAndRenderBuildings = async () => {
    ...
    if (!buildings || buildings.length === 0) { toggleSkeletonState(false); return; }
  ...
  const fetchBuildingDetails = async (buildingId) => {
    ...
    if (DOM.detailFilesList) {
      ...
      if (files.length === 0) DOM.detailFilesList.innerHTML = `<div style="border:1px dashed var(--border);background-color:var(--surface);padding:1.5rem;text-align:center;font-family:var(--font-mono);font-size:10px;text-transform:uppercase;color:var(--muted-foreground)">No documents attached</div>`;
      else {
    ...
    if (DOM.detailRoomsList) {
      ...
      if (currentBuildingRooms.length === 0) DOM.detailRoomsList.innerHTML = `<div style="border:1px dashed var(--border);background-color:var(--surface);padding:1.5rem;text-align:center;font-family:var(--font-mono);font-size:10px;text-transform:uppercase;color:var(--muted-foreground)">No rooms initialized yet</div>`;
      else {
  ...
  const bindUIEvents = () => {
    ...
    if(DOM.formNewProperty) DOM.formNewProperty.addEventListener('submit', async (e) => { e.preventDefault(); if(DOM.btnSubmitProperty) { DOM.btnSubmitProperty.textContent = "Registering..."; DOM.btnSubmitProperty.disabled = true; } const { error } = await supabase.from('buildings').insert([{ building_code: document.getElementById('input-building-code')?.value || 'PRJ', address_line_1: document.getElementById('input-address')?.value || 'Unknown', state: document.getElementById('input-state')?.value || 'WA', status: document.getElementById('input-status')?.value || 'Baseline Pending' }]); if (!error) { DOM.formNewProperty.reset(); switchView('registry'); fetchAndRenderBuildings(); } });
    ...
    if(DOM.formRoomManage) DOM.formRoomManage.addEventListener('submit', async (e) => { e.preventDefault(); if(DOM.btnSaveRoom) { DOM.btnSaveRoom.textContent = 'Saving...'; DOM.btnSaveRoom.disabled = true; } const roomId = document.getElementById('edit-room-id')?.value; const payload = { room_code: document.getElementById('edit-room-code')?.value, room_name_current: document.getElementById('edit-room-name')?.value, map_x: document.getElementById('edit-room-x')?.value ? Number(document.getElementById('edit-room-x').value) : null, map_y: document.getElementById('edit-room-y')?.value ? Number(document.getElementById('edit-room-y').value) : null }; if (roomId) { await supabase.from('rooms').update(payload).eq('id', roomId); } else { const { data: newRoom } = await supabase.from('rooms').insert({ building_id: currentBuildingContext.id, ...payload }).select().single(); if (newRoom) { const { data: pts } = await supabase.from('measurement_points').insert([ { building_id: currentBuildingContext.id, room_id: newRoom.id, element_code: 'RH' }, { building_id: currentBuildingContext.id, room_id: newRoom.id, element_code: 'CO2' }, { building_id: currentBuildingContext.id, room_id: newRoom.id, element_code: 'VOC' } ]).select(); if (pts) { await supabase.from('measurements').insert(pts.map(pt => ({ building_id: currentBuildingContext.id, measurement_point_id: pt.id, room_id: newRoom.id, status_flag: 'unknown', value: 0 }))); } } } if(DOM.modalRoomManage) DOM.modalRoomManage.classList.add('jb-hidden'); await fetchBuildingDetails(currentBuildingContext.id); });
    bindMapClick(); 
  };
  ...
  const init = async () => {
    ...
    try { await fetchAndRenderBuildings(); } catch (error) {}
  };
  return { init };
})();
