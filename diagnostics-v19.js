 ...
 * Diagnostics Controller (v18 - Database Join Fix & UI Unblocking)
 */
console.log("JB Diagnostics Controller v18 initializing...");
...
const JoeBuildsDiagnostics = (() => {
  ...
  const fetchDiagnosticsData = async (buildingId) => { 
    // FIX: Using *, measurement_points(*) ensures Supabase has the foreign keys required to perform the join.
    ...
    if (measurementsRes.error) {
      console.error("Supabase Measurements Error:", measurementsRes.error);
    }
  ...
  const mapToDOM = (data) => {
    ...
    // FIX: Update the Header UI *before* doing any measurement checks so it never gets stuck on "Loading..."
    ...
    // If there are truly no measurements, show the empty state message.
    if (data.measurements.length === 0) {
      ...
      if (DOM.tbodyAir) DOM.tbodyAir.innerHTML = `<tr><td colspan="5" style="color:var(--status-review); padding: 2rem;">No diagnostic measurements found in database for this property.</td></tr>`;
    ...
    data.measurements.forEach(m => { 
      ...
      if (m.measurement_points) { 
        // Handle both object and array returns from PostgREST safely
    ...
    if (thermals.length === 0 && DOM.tbodyThermal) {
      DOM.tbodyThermal.innerHTML = `<tr><td colspan="5" style="color:var(--muted-foreground); padding:1rem;">No thermal data available.</td></tr>`;
    }
    
    if (airTights.length === 0 && DOM.tbodyAir) {
      DOM.tbodyAir.innerHTML = `<tr><td colspan="5" style="color:var(--muted-foreground); padding:1rem;">No air tightness data available.</td></tr>`;
    }
