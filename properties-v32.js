/**
 * Joe Builds - Properties Registry Controller (v32 - Error Logs & File Verification)
 */
console.log("JB Properties Controller v32 initializing...");
const JoeBuildsProperties = (() => {
  ...
  const handleFileUpload = async (e) => {
    ...
    if (targetRoomId) {
        if (!isImage) { showToast('Room evidence must be an image', 'error'); DOM.fileInput.value = ''; return; }
        ...
        if (!isDoc && !isImage) { showToast('Invalid file type for report', 'error'); DOM.fileInput.value = ''; return; }
