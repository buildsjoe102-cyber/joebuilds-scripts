/**
 * Joe Builds - Properties Registry Controller (v24 - Delete Property Feature)
 */
console.log("JB Properties Controller v24 initializing...");
const JoeBuildsProperties = (() => {
  ...
  const authenticateOperator = async () => {
    ...
    if (profile.role === 'client' || profile.role === 'demo' || profile.role === 'suspended') { window.location.href = '/dashboard'; return null; }
    return profile;
  };
  ...
  const openEditPropertyModal = () => {
    ...
    document.getElementById('edit-building-code').value = currentBuildingContext.building_code || ''; document.getElementById('edit-address').value = currentBuildingContext.address_line_1 || ''; document.getElementById('edit-state').value = currentBuildingContext.state || ''; document.getElementById('edit-status').value = currentBuildingContext.status || ''; document.getElementById('edit-property-type').value = currentBuildingContext.property_type || '';
    
    // Inject Dynamic Delete Button if it doesn't exist
    if (!document.getElementById('btnDeleteProperty')) {
      const form = document.getElementById('formEditProperty');
      const delBtn = document.createElement('button');
      delBtn.id = 'btnDeleteProperty'; delBtn.type = 'button';
      delBtn.className = 'btn-secondary'; // Matches UI styling
      delBtn.style.marginTop = '0.5rem'; delBtn.style.width = '100%';
      delBtn.style.color = 'var(--status-review)'; delBtn.style.borderColor = 'var(--status-review)';
      ...
      delBtn.addEventListener('click', async () => {
        ...
        if (confirmDelete) {
          ...
          delBtn.textContent = 'Delete Property'; delBtn.disabled = false;
        }
      });
    }
  ...
  const handleSaveProperty = async () => {
    ...
    const payload = { building_code: document.getElementById('edit-building-code').value, address_line_1: document.getElementById('edit-address').value, state: document.getElementById('edit-state').value, status: document.getElementById('edit-status').value, property_type: document.getElementById('edit-property-type').value };
  ...
  const handleFileUpload = async (e) => {
    ...
    if (uploadErr) { if(DOM.uploadStatusText) DOM.uploadStatusText.textContent = `Drop or click to upload`; showToast('Evidence upload failed', 'error'); } else { const { data: urlData } = supabase.storage.from('property_assets').getPublicUrl(fileName); await supabase.from('evidence_assets').insert([{ building_id: currentBuildingContext.id, file_name: file.name, file_url: urlData.publicUrl }]); if(DOM.uploadStatusText) DOM.uploadStatusText.textContent = `Drop or click to upload`; showToast('Evidence uploaded successfully'); fetchBuildingDetails(currentBuildingContext.id); }
  ...
  const fetchBuildingDetails = async (buildingId) => {
    const [filesRes, roomsRes] = await Promise.all([ supabase.from('evidence_assets').select('*').eq('building_id', buildingId).order('created_at', { ascending: false }), supabase.from('rooms').select('*').eq('building_id', buildingId).order('created_at', { ascending: true }) ]);
