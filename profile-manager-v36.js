 ...
 * Global Profile Manager (v36 - Dropdown Property Switcher & Persistence)
 */
const JoeBuildsProfileManager = (() => {
  ...
  const bindClientPropertySwitcher = async (profile) => {
    // Only process for actual clients. Admins/Operators use the Admin interface.
    ...
    if (validAccesses.length > 1) {
       ...
       if (operatorMenu && !document.getElementById('clientPropertySwitcher')) {
           let optionsHtml = '';
           ...
           // Inject the dropdown directly into the top of the Operator Menu
           const switcherHtml = `
           <div style="padding: 0.75rem 1rem; border-bottom: 1px solid var(--border); background: var(--surface);">
           ...
           document.getElementById('clientPropertySwitcher').addEventListener('change', async (e) => {
               ...
               // Safely persist the new building to the user's main profile
               await supabaseClient.from('profiles').update({ building_id: newBid }).eq('memberstack_id', currentMember.data.id);
               
               // Reload the page to securely pull all new data from Supabase across all elements
               window.location.reload(); 
           });
       }
    }
