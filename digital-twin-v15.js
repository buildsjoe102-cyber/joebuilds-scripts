/**
 * Joe Builds - Digital Twin Controller (v15 - Demo Support)
 */
const JoeBuildsDigitalTwin = (() => {
  ...
  const fetchTwinData = async (buildingId) => {
    const [buildingRes, projectsRes, roomsRes, measurementsRes, evidenceRes] = await Promise.all([
      supabase.from('buildings').select('*').eq('id', buildingId).single(), supabase.from('projects').select('*').eq('building_id', buildingId).order('created_at', { ascending: false }), supabase.from('rooms').select('*').eq('building_id', buildingId).order('created_at', { ascending: true }), supabase.from('measurements').select(`*, measurement_points(zone_code, element_code)`).eq('building_id', buildingId), supabase.from('evidence_assets').select('*').eq('building_id', buildingId).order('created_at', { ascending: false })
    ]);
  ...
  const renderDashboard = (data) => {
    ...
    data.rooms.forEach(room => {
      ...
      const statuses = roomMs.map(m => m.status_flag); let overallStatus = 'stable'; if (statuses.includes('review')) overallStatus = 'review'; else if (statuses.includes('risk')) overallStatus = 'risk'; else if (statuses.includes('measured')) overallStatus = 'measured';
      ...
      activeZonesData[room.id] = { id: room.id, title: room.room_name_current, code: room.room_code.toLowerCase(), map_x: room.map_x, map_y: room.map_y, rh: getVal('RH'), tvoc: getVal('VOC'), co2: getVal('CO2'), status: overallStatus, badge: overallStatus === 'risk' ? 'At Risk' : overallStatus === 'review' ? 'Review Required' : overallStatus.charAt(0).toUpperCase() + overallStatus.slice(1), notes: room.notes || 'No active analyst notes for this zone.', monitoring: activeSensors.length > 0 ? `Active Sensors: ${activeSensors.join(', ')} · 30s interval` : `No active telemetry nodes linked.` };
    });

    const zoneKeys = Object.keys(activeZonesData);
    if (DOM.zonesCountLabel) DOM.zonesCountLabel.textContent = `Zones · ${zoneKeys.length}`;
    if (DOM.zonesListContainer) { DOM.zonesListContainer.innerHTML = ''; zoneKeys.forEach(key => { const zone = activeZonesData[key]; const btn = document.createElement('button'); btn.className = 'jb-zone-row-item'; btn.setAttribute('data-id', key); btn.innerHTML = `<span class="jb-zone-label">${zone.title}</span><span class="jb-status-badge jb-status-${zone.status}"><span class="jb-badge-dot bg-${zone.status}"></span>${zone.badge}</span>`; btn.addEventListener('click', (e) => { e.stopPropagation(); openDiagnosticPanel(key); }); DOM.zonesListContainer.appendChild(btn); }); }
    if (DOM.dynamicHotspots) { DOM.dynamicHotspots.innerHTML = ''; let unknownOffset = 10; zoneKeys.forEach(key => { const zone = activeZonesData[key]; let coords = null; if (zone.map_x != null && zone.map_y != null) { coords = { left: `${zone.map_x}%`, top: `${zone.map_y}%` }; } else if (roomCoordinates[zone.code]) { coords = roomCoordinates[zone.code]; } else { coords = { left: `${unknownOffset}%`, top: '90%' }; unknownOffset += 15; } const hotspotHTML = `<button class="jb-hotspot" style="left:${coords.left}; top:${coords.top};" data-id="${key}"><span class="jb-hotspot-pulse"><span class="jb-hotspot-ping-wave bg-${zone.status}"></span><span class="jb-hotspot-core-ring" style="border-color: var(--status-${zone.status});"></span></span><span class="jb-hotspot-tag">${zone.title}</span></button>`; DOM.dynamicHotspots.insertAdjacentHTML('beforeend', hotspotHTML); }); document.querySelectorAll('.jb-hotspot').forEach(node => { node.addEventListener('click', (e) => { e.stopPropagation(); openDiagnosticPanel(node.getAttribute('data-id')); }); }); }
  ...
  const openDiagnosticPanel = (zoneId) => {
    ...
    if (DOM.imageryGrid) { const roomImages = globalImages.filter(img => img.room_id === data.id); DOM.imageryGrid.innerHTML = ''; if (roomImages.length > 0) { roomImages.slice(0, 2).forEach(img => { DOM.imageryGrid.innerHTML += `<div class="jb-imagery-box" style="padding:0; overflow:hidden; border:none; background:var(--border);"><img src="${img.file_url}" style="width:100%; height:100%; object-fit:cover;"></div>`; }); } else { DOM.imageryGrid.innerHTML = `<div class="jb-imagery-box">No Img</div>`; } }
    const tNode = document.querySelector(`.jb-hotspot[data-id="${zoneId}"]`); const tRow = document.querySelector(`.jb-zone-row-item[data-id="${zoneId}"]`); if (tNode) tNode.classList.add('jb-active-node'); if (tRow) { tRow.classList.add('jb-active-node'); tRow.scrollIntoView({ behavior: 'smooth', block: 'nearest' }); }
  ...
  const init = async () => {
    ...
    try {
      ...
      if (targetBuildingId) { const twinData = await fetchTwinData(targetBuildingId); renderDashboard(twinData); } else { toggleSkeletonState(false); }
    } catch (error) {}
  };
  return { init };
})();
