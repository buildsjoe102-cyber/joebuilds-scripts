/**
 * Joe Builds - Admin Controller (v28 - Session Linking)
 */
...
const JoeBuildsAdmin = (() => {
  ...
  const bindDeleteBtns = (container) => {
    container.querySelectorAll('.btn-del-record').forEach(btn => {
      btn.addEventListener('click', async (e) => {
        ...
        if(confirm("Permanently delete this record?")) { btn.disabled = true; await supabase.from(table).delete().eq(col, id); showToast('Record deleted'); loadAssetContext(currentBuildingId); }
      });
    });
  };
  ...
  const renderTwinEditor = (rooms, measurements, sessions) => {
    ...
    DOM.twinNodesContainer.querySelectorAll('input[data-table="measurements"], select[data-table="measurements"]').forEach(input => { 
      input.addEventListener('change', (e) => { const p = {}; p[e.target.getAttribute('data-col')] = e.target.value; autoSaveData('measurements', 'id', e.target.getAttribute('data-id'), p, e.target.closest('.card-surface')); }); 
    });

    // Special listener for assigning all room measurements to a single session
