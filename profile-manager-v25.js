 ...
 * Global Profile Manager (v25 - Comprehensive Knowledge Base Update)
 */
const JoeBuildsProfileManager = (() => {
  ...
  const bindProfileEvents = (profile) => {
    ...
    if (isDemoActive && !document.getElementById('jbDemoBanner')) {
      const demoBannerHtml = `<div id="jbDemoBanner" style="position:fixed; bottom: 5rem; left: 50%; transform: translateX(-50%); background: var(--surface, #fff); color: var(--foreground, #000); padding: 12px 24px; border: 1px solid var(--status-review); z-index: 100000; display:flex; align-items:center; gap: 12px; font-family: var(--font-mono); font-size: 10px; text-transform:uppercase; letter-spacing: 0.1em; box-shadow: 0 10px 25px rgba(0,0,0,0.15); border-radius: 4px;">
  ...
  // ==========================================
  // BRAND NEW COMPREHENSIVE KNOWLEDGE BASE
  // ==========================================
  const getHelpContent = (role) => {
    if (role === 'admin' || role === 'operator') {
      return `
        <div class="jb-accordion-item is-open">
          <button class="jb-accordion-header">Auto-Save & Custom Dropdowns <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m6 9 6 6 6-6"></path></svg></button>
          <div class="jb-accordion-content">
            <strong>Auto-Save:</strong> Measurement inputs and pathway statuses save automatically when you click away or press Enter. The card flashes green when saved.<br><br>
            <strong>Custom Dropdowns:</strong> On the Properties page, you can select from the dropdown OR type a completely new State, Status, or Property Type. The system will learn it and save it for next time.
          </div>
        </div>
        <div class="jb-accordion-item">
          <button class="jb-accordion-header">Property & Room Management <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m6 9 6 6 6-6"></path></svg></button>
          <div class="jb-accordion-content">
            <strong>Creating/Editing:</strong> Register new homes via the Managed Properties page. Click any property to add Rooms (Spatial Nodes). Click the floorplan map to set a node's exact coordinates.<br><br>
            <strong>Deleting:</strong> You can now delete Rooms or entire Properties via their respective "Edit" menus. <em style="color:var(--status-review);">Warning: Deleting a property permanently destroys all associated measurements, rooms, and reports.</em>
          </div>
        </div>
        <div class="jb-accordion-item">
          <button class="jb-accordion-header">User Access & Demo Mode <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m6 9 6 6 6-6"></path></svg></button>
          <div class="jb-accordion-content">
            <strong>User Management (Admins):</strong> Click the "Users" button in the header. You can assign properties to clients, elevate roles (Client, Operator, Admin), <strong>Suspend</strong> access, or delete their database profile completely.<br><br>
            <strong>Demo Mode:</strong> If you or a client don't have a property assigned, the Operator dropdown will show "Explore Demo Property". This loads a safe, fully-populated dummy property (DEMO-001) for exploration.
          </div>
        </div>
        <div class="jb-accordion-item">
          <button class="jb-accordion-header">Contextual Document Uploads <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m6 9 6 6 6-6"></path></svg></button>
          <div class="jb-accordion-content">
            Files dragged into the <strong>Admin Page Uploader</strong> go directly to the client's Reports center.<br><br>
            <strong>Target Context:</strong> If you select a specific room from the dropdown before uploading an image (JPG/PNG), the system attaches it to that room's Home Map node instead of the general reports center.
          </div>
        </div>
      `;
    } else {
      return `
        <div class="jb-accordion-item is-open">
          <button class="jb-accordion-header">Understanding Your Measured Home Record <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m6 9 6 6 6-6"></path></svg></button>
          <div class="jb-accordion-content">
            Your dashboard provides a high-level health summary. <br><br>
            <strong>Integrity Index:</strong> The badge in the top right is a composite score out of 1.00 indicating overall envelope resilience.<br><br>
            <strong>Next Action:</strong> The dark green panel highlights the exact next step required in your controlled upgrade sequence.
          </div>
        </div>
        <div class="jb-accordion-item">
          <button class="jb-accordion-header">Home Map & Diagnostic Records <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m6 9 6 6 6-6"></path></svg></button>
          <div class="jb-accordion-content">
            <strong>Home Map:</strong> An interactive 2D map. Tap any circular room node to open the side-drawer and inspect measurement data (like CO₂ levels) and room-specific thermal imagery.<br><br>
            <strong>Diagnostic Record:</strong> The raw data ledger logging exact measurements. Click any row to pull up the specific instrument record and analyst notes.
          </div>
        </div>
        <div class="jb-accordion-item">
          <button class="jb-accordion-header">Reports & Stability Tracking <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m6 9 6 6 6-6"></path></svg></button>
          <div class="jb-accordion-content">
            The <strong>Reports</strong> page houses all your downloadable engineering PDFs. The calendar displays your <strong>Stability Check Cycle</strong>, marking past audits and upcoming seasonal stress tests.
          </div>
        </div>
        <div class="jb-accordion-item">
          <button class="jb-accordion-header">Building Science Glossary <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m6 9 6 6 6-6"></path></svg></button>
          <div class="jb-accordion-content">
            <ul>
              <li><strong>ACH50:</strong> Air Changes per Hour at 50 Pascals. Measures how drafty or airtight the home is (Lower is better).</li>
              <li><strong>U-Value:</strong> Measures thermal transmittance (heat loss). Lower is better.</li>
              <li><strong>VOC:</strong> Volatile Organic Compounds. Harmful gasses emitted by paints and materials.</li>
              <li><strong>RH:</strong> Relative Humidity. High RH leads to condensation and mold risk.</li>
            </ul>
          </div>
        </div>
      `;
    ...
    if (window.supabase) {
      ...
      try {
        ...
        if (currentMember && currentMember.data) {
          ...
          const sideMeta = document.querySelector('.sidebar-footer .text-muted-foreground, #desktopSidebarMeta');
          if (sideMeta && sideMeta.textContent.includes('Authenticating')) sideMeta.textContent = 'Active Session';

          const { data: profile } = await supabaseClient.from('profiles').select('*').eq('memberstack_id', currentMember.data.id).single();
          ...
          setTimeout(() => { document.body.classList.add('jb-data-ready'); }, 300);
        }
      } catch (err) {
