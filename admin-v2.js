 ...
 * Administrative Data Controller (v2 - Full Fixes & Skeleton Loader)
 */
const JoeBuildsAdmin = (() => {
  ...
  const authenticateAdmin = async () => {
    ...
    // BRUTE FORCE REVEAL: Override Webflow CSS globally
  ...
  const autoSaveData = async (table, id, payload, elementToFlash) => {
    if (!id || id === 'new') return; // Prevents crashing if no ID exists yet
  ...
  const renderTwinEditor = (rooms, measurements) => {
    ...
    rooms.forEach(room => {
      // Improved matching logic for room IDs
      ...
      const getValInput = (code) => { 
        const sm = roomMs.find(x => x.measurement_points?.element_code === code); 
