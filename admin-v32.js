/**
 * Joe Builds - Admin Controller (v32 - Fully Restored & Crash Proof)
 */
console.log("JB Admin Controller v32 initializing...");
const JoeBuildsAdmin = (() => {
  ...
  const showErrorOnScreen = (message, errDetail = "") => {
    ...
    errorBox.innerHTML = `<h3 style="margin-top:0; color:white;">Admin Crash Report</h3><p>${message}</p><p style="font-size: 10px; background: rgba(0,0,0,0.2); padding: 8px;">${errDetail}</p><button onclick="this.parentElement.remove()" style="background: white; color: black; border: none; padding: 5px 10px; cursor: pointer; margin-top: 10px;">Close</button>`;
  ...
  const loadAssetContext = async (buildingId) => {
    ...
    } catch(err) { showErrorOnScreen("Error Loading Asset Context", err.message || err); }
  ...
  const init = async () => {
    try {
      ...
      if (!window.supabase) { showErrorOnScreen("Supabase SDK missing"); return; }
      ...
      const profile = await authenticateAdmin(); if (profile) await loadGlobalAssets();
    } catch (error) {
      showErrorOnScreen("Admin Initialization Crash", error.message || error);
    }
  };
  return { init };
})();
