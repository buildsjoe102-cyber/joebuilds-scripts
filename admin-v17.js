/**
 * Joe Builds - Admin Controller (v17 - Header/Sidebar Context Fix)
 */
const JoeBuildsAdmin = (() => {
  ...
  const authenticateAdmin = async () => {
    ...
    currentUserRole = profile.role || 'client'; if (currentUserRole === 'client' || currentUserRole === 'demo') { window.location.href = '/dashboard'; return null; }
  ...
  const loadAssetContext = async (buildingId) => {
    try {
      ...
      // FIX: Update Sidebar and Header UI 
      if (bData) {
  ...
  const openUsersModal = async () => {
    if(!DOM.usersTableBody || !DOM.usersModal) return; DOM.usersTableBody.innerHTML = `<tr><td colspan="4" style="text-align:center; padding: 2rem;">Loading security records...</td></tr>`; DOM.usersModal.classList.remove('jb-hidden');
    ...
    profiles.forEach(p => {
      const nameStr = p.first_name || 'User'; const emailStr = p.email || (p.memberstack_id ? p.memberstack_id.substring(0,8) : 'Unknown'); const roleDisabled = currentUserRole !== 'admin' ? 'disabled title="Only Admins can modify roles"' : '';
      const tr = document.createElement('tr');
      tr.innerHTML = `<td style="font-weight: 500;">${nameStr}</td><td style="color:var(--muted-foreground); font-size:11px;">${emailStr}</td><td><select class="control-select" data-pid="${p.id}" data-type="role" style="padding: 0.25rem;" ${roleDisabled}><option value="client" ${p.role === 'client' ? 'selected' : ''}>Client</option><option value="demo" ${p.role === 'demo' ? 'selected' : ''}>Demo</option><option value="operator" ${p.role === 'operator' ? 'selected' : ''}>Operator</option><option value="admin" ${p.role === 'admin' ? 'selected' : ''}>Admin</option></select></td><td><select class="control-select" data-pid="${p.id}" data-type="building" style="padding: 0.25rem;">${bOptions}</select></td>`;
      if (p.building_id) { const bSelect = tr.querySelector(`select[data-type="building"]`); if(bSelect) bSelect.value = p.building_id; }
      tr.querySelectorAll('select').forEach(sel => { sel.addEventListener('change', async (e) => { e.target.style.borderColor = 'var(--status-stable)'; const payload = {}; if (e.target.dataset.type === 'role') payload.role = e.target.value; if (e.target.dataset.type === 'building') payload.building_id = e.target.value || null; await supabase.from('profiles').update(payload).eq('id', e.target.dataset.pid); setTimeout(() => e.target.style.borderColor = 'var(--border)', 500); }); }); DOM.usersTableBody.appendChild(tr);
    });
  };
  ...
  const init = async () => {
    ...
    // Quick DOM assignments for header/sidebar so it doesn't stay stuck on "Loading..."
