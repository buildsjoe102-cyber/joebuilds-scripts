 ...
 * Dashboard Controller (v18 - Sidebar DOM Fix)
 */
const JoeBuildsDashboard = (() => {
  ...
  const init = async () => {
    // Highly robust DOM selectors to catch every possible variation of the sidebar footer
    DOM = { 
    ...
    try {
      ...
      if (targetBuildingId) {
        ...
        if (data.building) {
          ...
          // Replaces "Authenticating" on the dashboard
