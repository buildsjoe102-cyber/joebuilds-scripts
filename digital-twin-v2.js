 ...
 * Unified Digital Twin Controller (v2 - Skeleton Loader, Smart Hotspots, Dynamic Header)
 */
const JoeBuildsDigitalTwin = (() => {
  const SUPABASE_URL = 'https://jsqyfiwkbuvuajwzbjhd.supabase.co';
  const SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImpzcXlmaXdrYnV2dWFqd3piamhkIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODE2MzY0MDEsImV4cCI6MjA5NzIxMjQwMX0.F315XwWSxPHEoCjQ14VDfpLBSbH9poN94fMyBGXUehE'; // PASTE YOUR KEY HERE
  ...
  const DOM = {
    ...
    headerClimateText: document.querySelectorAll('.jb-desktop-telemetry .jb-tabular')[1], // Grabs the Climate text
    headerSysText: document.querySelectorAll('.jb-desktop-telemetry .jb-tabular')[2], // Grabs the Sys text
  ...
  const toggleSkeletonState = (isLoading) => {
    if (isLoading) {
      if (DOM.zonesCountLabel) DOM.zonesCountLabel.classList.add('jb-skeleton-block');
      if (DOM.desktopHeaderAsset) DOM.desktopHeaderAsset.classList.add('jb-skeleton-block');
      if (DOM.headerSysText) DOM.headerSysText.classList.add('jb-skeleton-block');
    } else {
      if (DOM.zonesCountLabel) DOM.zonesCountLabel.classList.remove('jb-skeleton-block');
      if (DOM.desktopHeaderAsset) DOM.desktopHeaderAsset.classList.remove('jb-skeleton-block');
      if (DOM.headerSysText) DOM.headerSysText.classList.remove('jb-skeleton-block');
    }
  };
  ...
  const renderDashboard = (data) => {
    ...
    // Optional: We can parse the state/postcode to guess the climate zone if desired later, for now we will leave it static.
    ...
    DOM.hotspots.forEach(h => { 
      ...
      if (!activeZonesData[code]) {
        h.style.display = 'none'; // Hide it from the map if DB has no record
      } else {
        h.style.display = 'flex'; // Ensure it is visible
        // Replace existing listeners to avoid duplicates
