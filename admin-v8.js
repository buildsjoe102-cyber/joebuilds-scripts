 ...
 * Administrative Data Controller (v8 - Hard Cache Bust & Diagnostics)
 */
const JoeBuildsAdmin = (() => {
  ...
  const authenticateAdmin = async () => {
    console.log("Admin Init Started - Verifying Memberstack...");
    const member = await window.$memberstackDom.getCurrentMember();
    if (!member || !member.data) {
      console.warn("No Memberstack session found.");
      window.location.href = '/login';
      return null;
    }

    console.log("Memberstack verified. Fetching Supabase Profile...");
    const { data: profile, error } = await supabase.from('profiles').select('*').eq('memberstack_id', member.data.id).single();
    
    if (error || !profile) {
      console.warn("Profile fetch error. Are RLS policies blocking reads?", error);
      window.location.href = '/login';
      return null;
    }
    ...
    if (role === 'client' || role === 'demo') { 
      console.log("User is a client. Kicking to dashboard.");
    ...
    if ((role === 'admin' || role === 'operator') && DOM.btnUserManagement) {
  ...
  const loadGlobalAssets = async () => {
    console.log("Loading all buildings from database...");
    ...
    console.log("Buildings loaded successfully. Loading Context for ID:", currentBuildingId);
    loadAssetContext(currentBuildingId); 
  };
  ...
  const loadAssetContext = async (buildingId) => {
    try {
      ...
      if (!bData) {
        console.warn("Building Data is Null");
        return;
      }
      ...
      if (mData.length === 0 && sData.length === 0) {
        console.log("Asset has no data. Showing Empty State.");
        ...
        console.log("Asset has data. Rendering Editors.");
  ...
  const handleReportUpload = async (e) => {
    ...
    if(DOM.uploadStatusText) DOM.uploadStatusText.textContent = `Drag & Drop Diagnostic Reports`; 
    if(DOM.reportInput) DOM.reportInput.value = '';
    if (!dbErr && DOM.archiveCount) DOM.archiveCount.textContent = `Archive currently holds ${(parseInt(DOM.archiveCount.textContent.match(/\d+/)[0]) || 0) + 1} reports.`;
  };
