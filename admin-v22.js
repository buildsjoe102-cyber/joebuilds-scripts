/**
 * Joe Builds - Admin Controller (v22 - Assessments & Issues Wired)
 */
const JoeBuildsAdmin = (() => {
  ...
  let currentRooms = []; // Stored to populate the issue room dropdowns
  ...
  const DOM = { 
    ...
    assessmentsContainer: document.getElementById('assessments-container'), issuesContainer: document.getElementById('issues-container'),
    btnNewAssessment: document.getElementById('btnNewAssessment'), btnNewIssue: document.getElementById('btnNewIssue')
  };
  ...
  const initUIEvents = () => {
    ...
    if (DOM.btnNewAssessment) DOM.btnNewAssessment.addEventListener('click', async () => {
       if(!currentBuildingId) return;
       await supabase.from('assessment_sessions').insert([{ building_id: currentBuildingId, assessment_type: 'New Session', status: 'Scheduled' }]);
       showToast('Session created'); loadAssetContext(currentBuildingId);
    });
    if (DOM.btnNewIssue) DOM.btnNewIssue.addEventListener('click', async () => {
       if(!currentBuildingId) return;
       await supabase.from('issues_findings').insert([{ building_id: currentBuildingId, issue_type: 'New Finding', status: 'Monitor' }]);
       showToast('Finding created'); loadAssetContext(currentBuildingId);
    });
  };
  ...
  const loadAssetContext = async (buildingId) => {
    try {
      ...
      if (mData.length === 0 && sData.length === 0 && sessData.length === 0) { 
        ...
        renderTelemetryEditor(mData); renderTwinEditor(currentRooms, mData); renderPathwayEditor(sData); 
      }
  ...
  const initializeBuildingData = async (bId) => {
    try {
      ...
      // Seed Initial Assessment Session
  ...
  const openUsersModal = async () => {
    ...
    profiles.forEach(p => {
      ...
      tr.querySelector('.btn-delete-user').addEventListener('click', async () => {
        const confirmDel = confirm(`Are you sure you want to delete ${emailStr}? This removes their database connection.`);
        if (confirmDel) {
  ...
  // --- NEW: RENDER ASSESSMENTS ---
  const renderAssessments = (sessions) => {
    if(!DOM.assessmentsContainer) return; DOM.assessmentsContainer.innerHTML = '';
    sessions.forEach(s => {
      const cardHTML = `
      <div class="card-surface" id="sess-${s.assessment_id}">
        <div class="card-header">
          <div class="card-title">Assessment Session</div>
          <button class="btn-del-record" data-table="assessment_sessions" data-colid="assessment_id" data-id="${s.assessment_id}" style="background:transparent;border:none;color:var(--status-review);cursor:pointer;padding:0.25rem;"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 6h18"></path><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path></svg></button>
        </div>
        <div class="metrics-input-grid">
          <select class="control-select" data-table="assessment_sessions" data-id="${s.assessment_id}" data-col="assessment_type">
             <option value="Home Performance Baseline" ${s.assessment_type === 'Home Performance Baseline' ? 'selected' : ''}>Baseline Check</option>
             <option value="90-Day Stability Check" ${s.assessment_type === '90-Day Stability Check' ? 'selected' : ''}>90-Day Check</option>
             <option value="Triggered Event Check" ${s.assessment_type === 'Triggered Event Check' ? 'selected' : ''}>Triggered Event</option>
             <option value="Verification" ${s.assessment_type === 'Verification' ? 'selected' : ''}>Verification</option>
          </select>
          <select class="control-select" data-table="assessment_sessions" data-id="${s.assessment_id}" data-col="status">
        ...
        <div class="metrics-input-grid" style="grid-template-columns: 1fr;">
          <input class="control-input" placeholder="Assessor Name" value="${s.assessor || ''}" data-table="assessment_sessions" data-id="${s.assessment_id}" data-col="assessor">
        </div>
      </div>`;
  ...
  const renderIssues = (issues) => {
    ...
    issues.forEach(i => {
      ...
      const cardHTML = `
      <div class="card-surface" id="issue-${i.issue_id}">
        <div class="card-header">
          <div class="card-title">Diagnostic Finding</div>
          <button class="btn-del-record" data-table="issues_findings" data-colid="issue_id" data-id="${i.issue_id}" style="background:transparent;border:none;color:var(--status-review);cursor:pointer;padding:0.25rem;"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 6h18"></path><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path></svg></button>
        </div>
        <div class="metrics-input-grid">
          <input class="control-input" placeholder="Issue Title" value="${i.issue_type || ''}" data-table="issues_findings" data-id="${i.issue_id}" data-col="issue_type">
          <select class="control-select" data-table="issues_findings" data-id="${i.issue_id}" data-col="status">
             <option value="Monitor" ${i.status === 'Monitor' ? 'selected' : ''}>Monitor</option>
             <option value="At Risk" ${i.status === 'At Risk' ? 'selected' : ''}>At Risk</option>
             <option value="Action Required" ${i.status === 'Action Required' ? 'selected' : ''}>Action Required</option>
             <option value="Closed" ${i.status === 'Closed' ? 'selected' : ''}>Closed</option>
          </select>
        </div>
        <select class="control-select" data-table="issues_findings" data-id="${i.issue_id}" data-col="room_id">${rSelected}</select>
        <textarea rows="2" class="control-textarea" placeholder="Client facing finding description..." data-table="issues_findings" data-id="${i.issue_id}" data-col="client_facing_wording">${i.client_facing_wording || ''}</textarea>
      </div>`;
  ...
  const renderTwinEditor = (rooms, measurements) => {
    ...
    bindAutoSave(DOM.twinNodesContainer, 'rooms', 'id'); // For room notes
    // We bind measurements independently so they save to the measurements table
  ...
  const handleReportUpload = async (e) => {
    ...
    const { error: uploadErr } = await storageClient.storage.from('reports').upload(fileName, file);
    ...
    if(DOM.reportInput) DOM.reportInput.value = ''; showToast('Report uploaded successfully'); loadAssetContext(currentBuildingId);
  };
