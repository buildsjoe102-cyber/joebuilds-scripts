 ...
 * Unified Dashboard Controller (v8 - Empty State Generator for Unassigned Clients)
 */
const JoeBuildsDashboard = (() => {
  ...
  const toggleSkeletonState = (isLoading) => {
    ...
    if (DOM.ctaTitle) elementsToToggle.push(DOM.ctaTitle); 
  ...
  const renderEmptyState = () => {
    ...
    if (DOM.matrixContainer) {
      ...
      DOM.matrixContainer.innerHTML = `
        <div style="text-align:center; padding: 6rem 2rem; background: var(--surface); border: 1px dashed var(--border); margin-bottom: 3rem;">
          <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1" stroke-linecap="round" stroke-linejoin="round" style="color:var(--muted-foreground); margin-bottom: 1rem;"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"></path><polyline points="9 22 9 12 15 12 15 22"></polyline></svg>
          <h3 style="font-size: 20px; font-weight: 500; margin-bottom: 0.5rem; color: var(--foreground);">Dashboard Empty</h3>
          <p style="color: var(--muted-foreground); font-size: 14px; max-width: 400px; margin: 0 auto;">Your dashboard will be populated with live telemetry and diagnostic data once an administrator assigns a property to your account.</p>
        </div>
      `;
    }
  };
  ...
  const init = async () => {
    ...
    try {
      ...
      if (profile?.building_id) {
        ...
        // NO BUILDING ASSIGNED! FIRE THE EMPTY STATE GENERATOR
        renderEmptyState();
        toggleSkeletonState(false);
      }
    } catch (error) {
