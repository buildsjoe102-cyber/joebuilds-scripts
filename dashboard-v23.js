 ...
 * Dashboard Controller (v23 - Strict ID Selectors for Header)
 */
const JoeBuildsDashboard = (() => {
  ...
  const init = async () => {
    // SECURE DOM MAPPING BY EXPLICIT ID
    DOM = { 
      ...
      // Absolute target IDs for header elements
    ...
    try {
      ...
      if (targetBuildingId) {
        ...
        if (data.building) {
          ...
          if (DOM.headerSysText) DOM.headerSysText.textContent = data.building.status || "Active Home Record";
